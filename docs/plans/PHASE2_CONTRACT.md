...
| Caps | Normal `storeCap` 524,288 with the C9.7.1 `base + 190` reserve; the C9.7.7 expanded regime raises it to 68,157,440 under its exact predicates only; physical read bound 68,157,441 in both regimes |
...
| SERVER_SNAPSHOT members | Ordinary only; requires `serverSnapshot`; forbids every operation, request, receipt and application member, and carries no `action` — it is reconstructed server authority rather than an operation, and a resume creates the next action's row rather than rewriting this one, so no immutable member is ever mutated |
...
| Application failure | A failing APPLYING obligation is re-run on each later readiness without a phase change and without a second server call (C9.7.6 L3640), for as long as it keeps failing. The contract states no retry bound and this store adds none: the row is never removed, never retired, and never inferred applied from server terminal state (C9.7.6 L3638, L3640). What the application *does* — its effects, its completion predicate and its crash-safe acknowledgement — is the per-action obligation of the C6.5 writer and is fixed with `functions/taskDisposition.js`; `applicationId` names the obligation and does not define it |

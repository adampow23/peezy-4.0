...
SERVER_SNAPSHOT = common + {serverSnapshot}                                            // phase:"SERVER_SNAPSHOT"; ordinary only (C9.7.7 L3674)
...
requestCanonicalJSON  = UTF-8(TaskCanonicalV1(<the exact validated callable request for this row's action>)); an immutable opaque string, stored and hashed but never traversed as a hidden object (C9.6.7 L3200). The per-action request member maps belong to the C6.5 writer and are fixed when `functions/taskDisposition.js` is authored; this store never decodes them
requestSHA256         = lowercase SHA-256(requestCanonicalJSON); the server's stored request_fingerprint is "op1_" + requestSHA256 and is never stored locally
receipt               = the complete exact stored server response map, retained opaquely; the C9.7.8 receiptSHA256 hashes exactly this map
...
| Rule | Exact statement |
|---|---|
| PREPARED members | Requires the operation and request members; forbids receipt, application and `serverSnapshot`. Durable before the dispatching await (C9.7.6 L3647) |
| DISPATCHED members | Same member set as PREPARED; the phase records that the call was issued, and it is the C9.7.8 pre-replay phase of both handoff families (L3709, L3710). Forbids receipt, application and `serverSnapshot` |
...
| SERVER_SNAPSHOT members | Ordinary only; requires `serverSnapshot`; forbids every operation, request, receipt and application member |
...
| Transitions (ordinary) | `PREPARED → DISPATCHED → RECEIPT → APPLYING → (removed)`. `SERVER_SNAPSHOT` is entered only by the C9.7.11 reconstruction; the exact-epoch resume of C9.7.11 L3793 transitions it to `PREPARED` for the next action under the same recovery key, which is the only edge out of it besides removal and the C9.7.7 L3675 one-for-one replacement. No other edge exists; any other stored pair is structural corruption |
| Transitions (cancel) | `PREFLIGHT → PREPARED → DISPATCHED → RECEIPT → APPLYING → (removed)`. A cancel row is removed only after its APPLYING obligation completes (C9.7.7 L3678) |
| Inspection outcomes | On the C9.7.8 `committed` outcome a DISPATCHED row becomes RECEIPT with the returned receipt. On `absent` it stays in `DISPATCHED`, its C9.7.8 pre-replay phase (L3709, L3710, L3723), and the write clears only the receipt and application members that phase forbids. `pending` is unreachable for both handoff families (C9.7.8 L3722) |
| Transport failure | A transport error, a malformed or noncanonical response, auth drift or digest drift writes nothing and leaves the row byte-identical in its current phase (C9.7.4 L3571) |
| Application failure | A failing APPLYING obligation is re-run on each later readiness without a phase change and without a second server call (C9.7.6 L3640). After the store's frozen retry bound it stops re-running and surfaces the fixed local `HANDOFF_APPLICATION_BLOCKED`, retaining every byte; the row is never removed on failure |
...
WorkflowRetiredDiagnosticV1 = {schemaVersion:1,workflowId,taskInstanceId,flowAttemptGeneration,code,createdAt}
   code = WORKFLOW_REQUEST_SUPERSEDED | WORKFLOW_REQUEST_ABANDONED | WORKFLOW_APPLICATION_BLOCKED   (closed set)

flowAttemptGeneration = safe nonnegative integer, incremented once per flow attempt of a task instance; it is the numeric member of the C9.7.6 recovery key and the C9.7.8 identity map
...
receipt               = the complete exact stored workflow outcome map, retained opaquely; the C9.7.8 receiptSHA256 hashes exactly this map, without the response-authority expansion
...
| Capacity | One cap, 16,777,216 complete canonical outer-envelope bytes, with the C9.7.1 `base + 190` reserve and a physical read bound of cap + 1. The expanded regime is handoff's alone (C9.7.1 L3472). `records` holds at most 32 elements and `retiredDiagnostics` at most 16; each complete record is at most 524,288 bytes. A write that would exceed any of these writes nothing, leaves the target byte-identical, and returns the fixed local `WORKFLOW_STORE_CAPACITY_EXCEEDED` with zero server call and zero opener call |
| PREPARED members | Requires `requestCanonicalJSON`; forbids receipt and application members. Durable before the dispatching await, because C9.7.13 names `prepare` and `dispatch` as distinct workflow operations and C9.7.6 L3647 forbids a mutating call before durability |
...
| Transitions | `PREPARED → DISPATCHED → RECEIPT → APPLYING → (removed)`. On the C9.7.8 `committed` outcome a DISPATCHED row becomes RECEIPT; on `absent` it stays in `DISPATCHED`, its pre-replay phase, clearing only the members that phase forbids; `pending` is unreachable (C9.7.8 L3722). A transport error, malformed response, auth drift or digest drift writes nothing (C9.7.4 L3571). No other edge exists |
| Application failure | As C9.7.18: re-run on each later readiness with no phase change and no second server call; after the frozen retry bound the row is retired to `retiredDiagnostics` with code `WORKFLOW_APPLICATION_BLOCKED` and removed from `records` in the same atomic write |
| Sort | `records` is ordered by `createdAt`, then unsigned-UTF-8 `uid`, then unsigned-UTF-8 `taskInstanceId`, then numeric `taskGenerationEpoch`, then unsigned-UTF-8 `workflowId`, then numeric `flowAttemptGeneration` — `createdAt` followed by the complete C9.7.6 recovery key, so the order is total. `retiredDiagnostics` is ordered by `createdAt`, then unsigned-UTF-8 `workflowId`, then unsigned-UTF-8 `taskInstanceId`, then numeric `flowAttemptGeneration`, then unsigned-UTF-8 `code`, which is total because those five members are unique within the array. A noncanonical order is structural corruption |
| Uniqueness | The C9.7.6 recovery key `(uid,taskInstanceId,taskGenerationEpoch,workflowId,flowAttemptGeneration)` is unique within `records`; a duplicate within one source is unresolved, never dropped. A `retiredDiagnostics` element is unique on its own five members |
| Diagnostics | `retiredDiagnostics` elements are not candidates: they never enter `pendingRecordCount` (C9.7.6 L3624), a quarantine's are omitted on recovery and a live valid target's are carried byte-identically (C9.7.6 L3644). An element is appended only when a record leaves `records` without completing — superseded by a later attempt generation for the same task instance, abandoned when its flow attempt is cleared, or blocked by a failed application — and carries no request bytes, no receipt, no answer content, and no account, path or provider identity. At the 16-element ceiling the oldest element by the frozen sort is dropped in the same write |
| Auth state | Each record carries its own `{uid,authEpochUUID,credentialRevision}`; there is no auth singleton, which is why C9.7.6 L3624 names only `retiredDiagnostics` as excluded. That per-record auth state is never dropped (C9.7.6 L3638) and a row is actionable only while signed in to its UID (C9.7.8 L3732) |
...
| S5-CD7 | `Peezy 4.0/Assessment/AssessmentModels/AssessmentDataManager.swift`, and the single call site in `Peezy 4.0/Assessment/AssessmentModels/AssessmentCoordinator.swift` | S1 core/reset owns the data manager and S4 owns the coordinator (C10.2) → S5 changes `computeDistanceAndInterstate()` to return the computed distance and interstate as an operation-local value instead of applying them to the shared manager, and changes the one `completeAssessment()` line that awaits it to apply that value only after the existing P1-R UID and load-generation guard. Nothing else in either file — no assessment question, storage, geocoding-provider, save-path or guard change, and no new test file: the falsifier is S4's existing `AppRootAuthRaceTests` guard pin, whose premise this makes true, and S4's final full-class review of that class confirms it. S1's owner reviews the data-manager diff and S4's the coordinator line at S5 close-out (S5-CD7) |

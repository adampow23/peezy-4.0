...
| UserDefaults/preferences | Eleven exact UID-scoped keys and conditional global first name | C2.2, C6.6 | `DurableStoreRecoveryTests` — `UID-interpolated preference keys are registry-complete` |
...
#### C9.5.24 Shared task surface state (S4-CD3)

```text
TaskDispositionSurfaceState =
   actionable(contract)                          // a policy-absent or policy-bearing nonterminal contract whose row buttons are enabled
 | readOnly(reason, contract?)                   // reason: "completed"|"dismissed"|"malformed_present"|"gate_nonclear"|"store_blocked"; contract absent only for "malformed_present"
 | superseded(SupersededPresentation)            // SupersededPresentation = {source:"v2"|"legacy_v1", supersededBy, supersededAt?, copy, detail:{kind:"DATE",at}?}: source "v2" carries copy "Replaced", supersededAt, detail and renders per C9.5.21 as `Replaced — {formatter(detail.at)}`; source "legacy_v1" carries copy = the stored visible_status_copy verbatim, no supersededAt, no detail, and renders that copy verbatim (C9.5.21: no separator, no date); both decoded by the C9.5.20 decoder
raw-input seam: `TaskDispositionSurface.state(rawContract: [String: Any], status: TaskStatus, gateProjection: AccountDeletionGateProjection, readiness: ReadinessVector) -> TaskDispositionSurfaceState`; `TasksStore` retains and hands the stored `dispositionContract` map unmodified (the card mapper's projection is never the input), so `schema_version`, `superseded_at`, and `visible_status_detail` reach the decoder
```

| Rule | Exact statement |
|---|---|
| Producer | `TaskDispositionSurface` (S4) is the sole producer; every task row, the Home queue, and the disposition views consume the union; no view decodes a stored contract itself |
| Superseded | A valid v2 record and a valid legacy v1 record (C9.5.20) are `superseded` with `source` naming which; a malformed superseded record is `readOnly("malformed_present", nil)` with the C9.5.20 `malformedPresent` behavior; never `actionable` |
| Gate | A nonclear deletion gate or a blocked required store maps every row to `readOnly` with the matching reason; the union carries no UID, identity, or digest |
| Ownership | D18–D21 production (`TaskDispositionSurface.swift`, the task row/list views) and their falsifiers are S4's; S4 contributes the D18–D21 cases to `TaskSupersessionTests.swift` under the C10.3 row |

#### C9.5.x Named falsifiers

| Test file | Named test / fixture family |
|---|---|
| `Peezy 4.0Tests/TaskDispositionSurfaceTests.swift` | `TaskDispositionSurfaceState`: every stored-contract shape to exactly one member; gate/store mapping; the tri-state/shared-surface matrix (S4-CD3) |
...
BlockedSnapshot =
 ...
 | {schemaVersion:1,store:"dose",state:"malformed",recoveryStateDigest,bytesSHA256,byteLength,
  availableActions:["quarantine_dose_bytes"]}   // S4-CD1: the `peezy.{uid}.dailyDose.v2` bytes fail the C9.5.16 grammar; every byte and every legacy key is untouched
```
...
| `dose/malformed` (S4-CD1) | Produced only for the current signed UID by `DailyDoseLocalStore.observeMalformed(uid:)` (non-nil iff `load(uid:) == .malformed`, C9.5.16); `bytesSHA256`/`byteLength` are the malformed v2 bytes' digest and length; no legacy-key value, no payload byte, no path enters the snapshot; it blocks only the reset's dose cleanup (C9.7.13), never reserve/bind/dispatch |
...
| dose (S4-CD1) | The dose store is classified solely by `DailyDoseLocalStore.load(uid:)` for the current signed UID (`.malformed` → `dose/malformed`, else ready); no C9.7.3 file step applies |
...
| `dose/malformed` (S4-CD1) | `quarantine_dose_bytes` | `RecoveryAttemptKey{kind:"dose_quarantine",recoveryStateDigest}`; digest match pre-call and post-await over the dose `RecoveryObservedStateV1` (C9.7.12); the v2 bytes reread and SHA-256 equal to `bytesSHA256` immediately before the first write | Performed by `DailyDoseLocalStore.quarantineMalformed(uid:expectedBytesSHA256:) -> quarantined \| drifted \| ioFailed` (the sole v2-key actor, C6.6, holding the dose `(RecoveryAttemptKey,Task)` slot; the coordinator invokes it and never touches the key) in this exact order: (1) append the malformed v2 bytes as one `Data` blob to the array under `peezy.{uid}.dailyDose.v2.quarantine` (absent → one-element array; present → append, never replace, no cap); (2) `synchronize()` on the store's injected `UserDefaults` must return true (`CFPreferencesAppSynchronize` for `.standard`); (3) reread the array and require its last element byte-equal to the malformed bytes; (4) only then remove `peezy.{uid}.dailyDose.v2`; (5) the same `synchronize()` must return true; (6) reread and require the v2 key absent. Any failure at (1)–(3) leaves the v2 key untouched and returns `ioFailed` (the store stays `dose/malformed` with a fresh digest); a failure at (5)–(6) after a successful (4) returns `ioFailed` and reclassification finds v2 absent (ready) or still present (repeat). The three legacy keys are never read, written, or removed; the next dose read finds v2 absent and the C9.5.16 bridge/floor rules apply; the quarantine key is read only by `observeMalformed` (count) and `quarantineMalformed` (step-3 verification), never decoded or surfaced, and removed only by the preference barrier (eleventh C6.6 key) | Crash between (1) and (4) leaves the v2 key and a quarantine copy: reclassification sees the same bytes with `quarantineCount` incremented and a fresh digest; the old token returns the refreshed snapshot (C9.7.2) and the action is offered again (a second append of the same bytes is harmless); crash after (4) leaves the quarantine copy only: the store is ready; the last copy of user state is never removed before its copy is verified on disk |
...
| handoff reconciliation cancel | `(uid,taskInstanceId,sessionId,reasonCode,requesterNamespace.installationId,requesterNamespace.authEpochUUID,storedNamespace.installationId,storedNamespace.authEpochUUID)` (S5-CD10: the two stored members are in the key because C9.7.9 L3749 replaces **per candidate**, so two lawful ordinary rows differing only in `authEpochUUID` — or only in `installationId` under `TAKEOVER` — replace into two PREFLIGHTs identical on every other member; without them the key collides and C9.7.6's uniqueness rule calls two required rows unresolved) |
...
RecoveryObservedStateV1 = {schemaVersion:1,store,baseState,target,quarantine,availableActions,keychain?,firebase?,auth?,mismatchIdentityDigest?}
   | {schemaVersion:1,store:"dose",baseState:"malformed",bytesSHA256,byteLength,quarantineCount,availableActions:["quarantine_dose_bytes"]}   // S4-CD1: the dose alternative; produced by `DailyDoseLocalStore.observeMalformed(uid:) -> RecoveryObservedStateV1?`, which returns it iff `load(uid:) == .malformed` (bytes reread, hashed, and the quarantine array counted in one call), else nil; the only observation API for the key; `quarantineCount` = number of blobs already held by `peezy.{uid}.dailyDose.v2.quarantine`
...
RecoveryAttemptKey = {kind:"recover"|"merge"|"discard"|"cleanup",recoveryStateDigest}
   ...
   | {kind:"dose_quarantine",recoveryStateDigest}   // S4-CD1
RecoveryAction = recover | discard_quarantine | retry_cleanup | merge | reconcile(mismatchIdentityDigest) | foreign_reconcile | resolve(resolutionDigest,choices) | retry(errorCode) | repair_installation_identity | quarantine_dose_bytes   // S4-CD5: maps 1:1 onto the attempt-key kinds above
```
...
| Quarantine dose bytes (S4-CD1) | `dose_quarantine` |
...
dose (S4-CD1): not a ReadinessVector member; `dose/malformed` is surfaced by the recovery surface and blocks only the reset's dose cleanup (`resetDose` throws `localDoseMalformed`, C9.5.16); reserve/bind/dispatch/application are gated by the four stores above only
```
...
| `RoomCaptureArtifactOwner` / `NarrationLease` (S4-CD7) | `NarrationLease = {leaseId (lowercase UUID), uid, gateGeneration, sessionId}`; `RoomCaptureArtifactOwner` (S4, inside `LocalPrivacyPurgeCoordinator.swift`) is the sole issuer: `acquire(uid:sessionId:) async -> NarrationLease?` returns a lease only while the deletion gate is `clear` for that UID and the current `GateGeneration`, else nil; `revalidate(_ lease:) -> Bool` is true only while the lease is outstanding and the gate generation is unchanged; `release(_ lease:)`; `revokeAll()` marks every outstanding lease invalid and awaits every in-flight transfer's cancellation before returning; `NarrationService.start(lease:)`, `pendingNarration`, and `pendingNarrationTranscript` hold only a `NarrationLease`; the transcript itself lives in the actor: `deposit(_ transcript: String, for lease:) -> Bool` (false and dropped when `revalidate` is false) and `materialize(for lease:) -> String?` (returns and clears the deposited transcript only while `revalidate` is true; nil otherwise); in-flight media transfers register with `register(transfer lease:, cancel: @Sendable () -> Void) -> TransferHandle` (`TransferHandle = {handleId (lowercase UUID), lease}`) at the S4-owned call site that awaits `InventoryStorageService.uploadFrames` (`InventorySessionManager.swift`, wrapping the existing call; `InventoryStorageService.swift` is not edited) and settle with `settle(_ handle:)`; `cancel` cancels the Swift `Task` awaiting `uploadFrames`, the call site calls `settle` when that Task exits by return or throw, so `revokeAll()` returns only after every registered upload loop has exited, and any object Storage already holds is the C4 late-upload family; every transfer completion and every narration materialization revalidates the lease after each await and drops its buffer on `false`; `revokeAll()` invalidates every lease, invokes every registered transfer's `cancel`, and awaits every registered transfer's settlement before returning; gate activation calls `revokeAll()` before the room_capture ack (C2.2 order), so cancellation completes before revocation returns and nothing materializes or is deposited after the ack; an object a cancelled upload had already written is the server's late-upload family (C4) |
...
| `RouteAccountDeletionPurging`, `HandoffAccountDeletionPurging`, `ResetAccountDeletionPurging`, `WorkflowAccountDeletionPurging` | four durable-store purge seams; `AppRouteInbox` sole conformer of `RouteAccountDeletionPurging` (serialized purge, no second route-envelope writer); `receive` serializes load with ingress and never synthesizes empty authority; `WorkflowService` (S2's file; S5 writes the conformance) is the workflow store owner and the sole conformer of `WorkflowAccountDeletionPurging` (S4-CD5) |
| `DurableStoreRecovering` (S4-CD5) | one protocol every durable-store owner conforms to — `observe() async -> RecoveryObservation` where `RecoveryObservation = observed(RecoveryObservedStateV1) \| unavailable({store,state:"storage_io_unavailable"\|"installation_authority_unavailable"\|"installation_authority_invalid",errorCode})` (the typed unavailable branch carries no file observation, C9.7.2); `classify(_ observation: RecoveryObservation) -> ready \| blocked(BlockedSnapshot)` (the C9.7.3 order over the owner's own files; pure); `perform(_ action: RecoveryAction, expecting: RecoveryExpectation) async -> RecoveryResult` where `RecoveryExpectation = digest(recoveryStateDigest) \| unavailable({store,state,errorCode})` (digest-bearing actions carry the displayed digest; `retry`/`repair_installation_identity` carry the unavailable token, C9.7.12 "Unavailable no-digest") for the C9.7.4 actions of that store (`recover`, `discard_quarantine`, `retry_cleanup`, `merge`, `reconcile`, `resolve`, `retry`, `repair_installation_identity`; `recover_epoch` stays on `ResetEpochConflictRecovering`); the single `(RecoveryAttemptKey,Task)` slot of C9.7.12 lives in the owner; conformers: `AppRouteInbox` (S7), `HandoffSessionStore` (S5), `ResetOperationRegistry` (S4 per S4-CD2), `WorkflowService` (S5); the coordinator (`DurableStoreRecoveryCoordinator`) drives every store only through this protocol and never opens, replaces, or unlinks an owner's file |
...
FirestoreRuntime.install(_ owner: FirestoreRuntimeOwner) -> InstallOutcome (S4-CD2): registry states `empty | installed(owner)`; install on `empty` → `installed` (the only state change); install on `installed` → the fixed `alreadyInstalled` with zero state change, whether or not a lease was issued (a lease exists only after installation, so "a call after a lease" is a second call); no replacement, no second runtime. Before installation the existing consumer signatures are unchanged: `provider.acquire()` throws the fixed `FirestoreRuntimeError.notInstalled`; `provider.isCurrent(_:)` is false; `provider.published()` and `FirestoreRuntime.firestore()` (nonthrowing, default-parameter sites) trap with the fixed message `FIRESTORE_RUNTIME_NOT_INSTALLED` — a programming error, since S7 installs before `AppRootView` is created (C9.7.13) and tests install before any acquisition; no consumer file is edited. `FirestoreRuntime.provider` survives as a registry-backed computed static (so no consumer file is edited), `FirestoreRuntime.shared` is the one process registry, and `Phase2ProductionRuntime.firestoreRuntimeOwner` its sole production caller; tests drive install-once through an injected `FirestoreRuntime.Registry` instance, one per test; the one mechanical change to a closed S1 case is authorized: `productionRuntimeLeaseTargetsTheEmulator` installs a `FirestoreRuntimeOwner` into `FirestoreRuntime.shared` when empty before its acquisition (`alreadyInstalled` tolerated); no other closed case changes
...
#### C9.7.17 Handoff durable file (S5-CD3)

```text
file    = PeezyHandoffAuthority-v1.json      (Application Support; protection completeUntilFirstUserAuthentication)
quarantine sibling = PeezyHandoffAuthority-v1.quarantine-v1.json
DurableFileEnvelopeV1 = {schemaVersion:1,fileKind:"HANDOFF_AUTHORITY_V1",generationId,payload:HandoffPayloadV1,recoveryReceipt?,sha256}
sha256 = 64 lowercase hex over canonical UTF-8 of the complete envelope with only sha256 omitted (includes recoveryReceipt when present)
generationId = fresh lowercase RFC 4122 UUID per committed replacement; envelope bytes = canonical JSON, no trailing newline
HandoffPayloadV1 = {installation:{installationId},auth:<signedAuth>,sessions:{records:[HandoffSessionRecordV1]},reconciliationCancels:[ReconciliationCancelV1]}
   exact four-member set; every member required; the empty state is the C9.7.5 canonical empty payload
```

| Rule | Exact statement |
|---|---|
| Owner | `HandoffSessionStore` is one process-wide Swift `actor`; sole reader/modify/replacer of both files; sole accessor of the C9.7.10 Keychain installation identity through `InstallationIdentityProviding`; no view, coordinator, callback, or façade opens either file |
| Startup accept | Only exact outer/member sets, `fileKind` `HANDOFF_AUTHORITY_V1`, canonical bytes, valid generation UUID, valid payload, and recomputed `sha256` load; any other target is corrupt |
| Write protocol | Temp file in the same directory, POSIX write loop, `fsync` file, atomic `rename`, `fsync` directory; C9.7.5's reentrancy rule applies to the whole sequence |
| Corrupt target | Never reset empty. The C9.7.3 Keychain precedence runs first: while the installation authority is unavailable or invalid the store performs **zero file mutation** and classifies `installation_authority_unavailable` or `installation_authority_invalid` (C9.7.3 L3536, C9.7.10 L3772/L3776). Only after that authority succeeds does the corrupt target rename to the quarantine sibling with a directory `fsync` and the actor enter `CORRUPT_BLOCKED`; an existing quarantine leaves both files and stays blocked (P1-N, C9.7.3 L3547) |
| Caps | Normal `storeCap` 524,288 with the C9.7.1 `base + 190` reserve; the C9.7.7 expanded regime raises it to 68,157,440 under its exact predicates only; physical read bound 68,157,441 in both regimes. C9.7.7 L3680's exhaustive prospective projection additionally needs a frozen per-action maximum for the `receipt`, `requestCanonicalJSON` and `serverSnapshot` strings. **Those three ceilings are S6's and land with the real response shapes** (S5-CD10, Decision 11): none is stated here, because a conservative bound — each record at the expanded regime's 524,288 — projects 32 rows to roughly 16 MB against a 524,288 cap and would make the normal regime unreachable, and inventing a tighter one is the failure the S4 review named. Until S6 sets them, S5 computes the projection over the rows' actual current bytes and the transitions it can itself take, and no S5 test asserts a prospective ceiling |
| Capacity failure | A write whose complete envelope would exceed the applicable cap writes nothing, leaves the target byte-identical, and returns the fixed local `HANDOFF_STORE_CAPACITY_EXCEEDED` — the "frozen local handoff-capacity diagnostic" of C9.7.7 L3666. It is a local return value, not a C9.7.2 blocked state, and it makes zero server call and zero opener call |
| Protection parity | The protection class matches the route store's (C9.3.6), because both files must be readable at the same point in launch as the readiness vector they gate |

#### C9.7.18 Handoff records (S5-CD1)

Scope: the client durable records of C9.7.17 only. The server `activeHandoff` and `taskInteractionState` shapes stay open to S6 (C9.3 L2026); nothing here defines or constrains them, and no member below is a decoded projection of one.

```text
HandoffSessionRecordV1  (payload.sessions.records[])
common   = {uid,installationId,authEpochUUID,taskDocumentId,taskInstanceId,sessionId,action,phase,createdAt,updatedAt}
PREPARED        = common + {operationId,requestCanonicalJSON,requestSHA256}            // phase:"PREPARED"
DISPATCHED      = common + {operationId,requestCanonicalJSON,requestSHA256}            // phase:"DISPATCHED"
RECEIPT         = common + {operationId,requestCanonicalJSON,requestSHA256,receipt}    // phase:"RECEIPT"
APPLYING        = common + {operationId,requestCanonicalJSON,requestSHA256,receipt,applicationId}   // phase:"APPLYING"
SERVER_SNAPSHOT = (common less `action`) + {serverSnapshot}                             // phase:"SERVER_SNAPSHOT"; ordinary only (C9.7.7 L3674)

ReconciliationCancelV1  (payload.reconciliationCancels[])
common   = {uid,taskDocumentId,taskInstanceId,sessionId,reasonCode,
            storedNamespace:{installationId,authEpochUUID},
            requesterNamespace:{installationId,authEpochUUID},
            phase,createdAt,updatedAt}
PREFLIGHT  = common                                                                    // cancel only
PREPARED   = common + {operationId,requestCanonicalJSON,requestSHA256}
DISPATCHED = common + {operationId,requestCanonicalJSON,requestSHA256}
RECEIPT    = common + {operationId,requestCanonicalJSON,requestSHA256,receipt}
APPLYING   = common + {operationId,requestCanonicalJSON,requestSHA256,receipt,applicationId}

action                = one of beginHandoff | acknowledgeHandoffOpened | recordHandoffReturned | continueHandoff | resolveHandoff   (the five HANDOFF-family actions; cancelHandoff is the HANDOFF_CANCEL family and never appears on an ordinary record)
reasonCode            = AUTH_EPOCH_CHANGED | TAKEOVER
requestCanonicalJSON  = UTF-8(TaskCanonicalV1(<the exact validated callable request for this row's action>)); an immutable opaque string, stored and hashed but never traversed as a hidden object (C9.6.6 L3200). The per-action request member maps belong to the C6.5 writer and are fixed when `functions/taskDisposition.js` is authored; this store never decodes them
requestSHA256         = lowercase SHA-256(requestCanonicalJSON); it is the `requestAuthority` C9.7.8 L3709 and L3710 match for both handoff families, used **raw and never prefixed**. The `"op1_" + request_sha256` form of C9.3.3 L1884 belongs to the `INTENT_CLAIM` document and is not this family's; assuming it here would fail every match
receipt               = UTF-8(TaskCanonicalV1(<the exact stored server response map>)); an immutable opaque string exactly like the other two, never decoded here. The C9.7.8 receiptSHA256 is the lowercase SHA-256 of exactly these bytes, so the digest has one preimage and hashing can never disagree with storage
validation            = what the store proves before installing C9.7.8 provenance, without decoding any opaque string. The inspection reads the operation document at `users/{uid}/taskPlanOperations/{operationId}` and accepts only kind `TASK_OPERATION`, state `COMMITTED`, and the action the row retained — the candidate's handoff action for `HANDOFF`, `cancelHandoff` for `HANDOFF_CANCEL` (C9.7.8 L3709, L3710). Authority is `{operationId}`, request authority is `{requestSHA256}`. The identity map is C9.7.8 L3698's `{kind:"handoff",action,uid,installationId,authEpochUUID,taskInstanceId,sessionId}` for an ordinary row and L3699's `{kind:"handoff_cancel",uid,taskInstanceId,sessionId,reasonCode,requesterInstallationId,requesterAuthEpochUUID}` for a cancel: `action` **is** an ordinary identity member, and `taskDocumentId` is **not** an identity member of either — this store carries it for presentation and route consumption and never contributes it to an identity digest. The live signed-auth tuple must equal the row's at the payload `auth` singleton's `credentialRevision`, and `receiptSHA256` must recompute over the stored receipt bytes. That is the complete request/identity/response/auth correspondence C9.7.8 L3692 requires; member-level request correspondence is the C6.5 writer's, proved server-side, and this store never needs it. Provenance is process-local and cleared on load (C9.7.8 L3691), so after any disk load the proof is re-established by the C9.7.4 L3571 `receipt_mismatch` reconcile, never read from the file
applicationId         = "hap1_" + first40(SHA-256(TaskCanonicalV1({uid,identity_digest:<the row's C9.7.8 identityDigest>,receipt_sha256:<the row's C9.7.8 receiptSHA256>})))
serverSnapshot        = the canonical server authority the C9.7.11 ordinary reconstruction copied, retained byte-for-byte as an immutable opaque string and never traversed here; its shape is S6's (C9.3 L2026)
credentialRevision    = not a record member; the row's revision is the `credentialRevision` of the payload's `auth` singleton (C9.7.9 L3742), which is what C9.7.8 L3687's provenance binds
createdAt, updatedAt  = canonical UTC RFC 3339 millisecond instants under the C9.5.4 `writeTime` rule, which governs this store unchanged
```

| Rule | Exact statement |
|---|---|
| PREPARED members | Requires the operation and request members; forbids receipt, application and `serverSnapshot`. Durable before the dispatching await; see the durable dispatch boundary rule for what does and does not support that (C9.7.6 L3647 does not) |
| DISPATCHED members | Same member set as PREPARED; the phase records that the call **may have been issued** — never that it was, and never that it committed. It is written before the invocation (see the durable dispatch boundary), so at the instant of that write no call has been made, and C9.7.11 L3816's no-commit drop exists exactly for the row whose call never committed. It is the C9.7.8 pre-replay phase of both handoff families (L3709, L3710). Forbids receipt, application and `serverSnapshot` |
| RECEIPT members | Requires the receipt; forbids `applicationId` and `serverSnapshot` |
| APPLYING members | Requires the receipt and `applicationId`; forbids `serverSnapshot`; the local application obligation is non-openable until it completes (C9.7.6 L3640) |
| SERVER_SNAPSHOT members | Ordinary only; requires `serverSnapshot`; forbids every operation, request, receipt and application member, and **carries no `action`** — it is reconstructed server authority, not an operation. Its own C9.7.6 L3613 recovery key **is** the identity key of the ordinary row it replaced, which is what C9.7.6 L3628 means by counting the reconstruction as `recovered` under the same key; no separate reference member is stored, because a duplicate of members `common` already carries could disagree with them. The owner's Decision 12 ruling — that the resumed action is read from the referenced row — is recorded and **not yet implementable**: the only presentation-time read C9.7.11 offers is L3796's `actionLabel`, which is `activeHandoff.presentation_snapshot.action_label` under a trimmed/bounded **safe-label** grammar, and L3811 appends ordinals for duplicate labels, so it is not injective onto the five actions of L4068 and cannot name one. `action` is also an identity member of C9.7.8 L3698's map, which a display label may not source. The resumed action therefore stays a **registered open decision**, and this section states no `resume` edge rather than inventing one (S5-CD10, owner ruling 2026-09-08, Decision 12) |
| PREFLIGHT members | Cancel only; forbids every operation, request, receipt and application member. Created only under C9.7.7 L3679's predicate, never by a general begin or preparation. An ordinary record has no PREFLIGHT phase, because every PREFLIGHT the contract creates carries a `reasonCode` (C9.7.9 L3749, C9.7.11 L3808/L3815) |
| Immutability | `uid`, `installationId`, `authEpochUUID`, `taskDocumentId`, `taskInstanceId`, `sessionId`, `action`, `reasonCode`, both namespaces, `operationId`, `requestCanonicalJSON`, `requestSHA256` and `createdAt` are immutable once written; only `phase`, `updatedAt` and the receipt/application members a transition adds may change |
| Transitions — the in-operation event table | Every edge below is named by its **event** and its **source phase**. Indexing by phase alone is what made an earlier draft forbid a required edge, so no phase-only statement of this table is lawful. **This table covers the store's own operation edges only, and is not a closed enumeration of every edge the record can take.** Every recovery-time removal, drop, replacement and reconstruction is the named C9.7.4 recovery action, C9.7.11 foreign-resolution choice or C9.7.9 replacement that produces it, and those rules are the authority for it — C9.7.4 L3569 says live rows are "never removed/changed except named reconciliation replacement", which is what makes the named action the instrument rather than a phase edge. In particular the C9.7.11 L3789 digest-bound signed-out/different-UID ordinary auth purge, C9.7.6 L3629's three handoff-only `dropped` classifications (PREPARED no-dispatch proof, DISPATCHED single-snapshot absence proof, explicit Continue-other-device), C9.7.11 L3807's Continue and L3808's Restart group collapses, and C9.7.9 L3749's per-candidate replacement of one old-epoch ordinary by one current-requester `AUTH_EPOCH_CHANGED` PREFLIGHT are all lawful and none of them is structural corruption. An edge that is in neither this table nor a named rule of C9.7.3, C9.7.4, C9.7.6, C9.7.7, C9.7.9 or C9.7.11 is structural corruption. **Ordinary.** `prepare` (none) → creates PREPARED. `dispatch` (PREPARED) → DISPATCHED. **`first response` (DISPATCHED) → RECEIPT**, the same-process response to the call this row issued, validated under the `validation` rule above and its provenance installed; this is the only edge that stores a receipt outside recovery, and C9.7.8 L3692's frozen first-response behavior governs it, with C9.7.8 L3731 forbidding any first-response notification, accounting or callback. `receipt_mismatch` `committed` (**RECEIPT, APPLYING**) → the exact RECEIPT or APPLYING replacement C9.7.4 L3571 names, with provenance installed. `receipt_mismatch` `absent` (**RECEIPT, APPLYING**) → DISPATCHED, the pre-replay phase of both handoff families (C9.7.8 L3709, L3710), the write clearing only the members that phase forbids and preserving every immutable byte (C9.7.8 L3723). The sources are exactly the two receipt-bearing phases and never DISPATCHED: C9.7.3 L3540 classifies only a *receipt-bearing* row as `receipt_mismatch`, and C9.7.8 L3730 makes every current-auth RECEIPT or APPLYING row loaded from disk one before any application, callback, opener, notification or accounting. `apply` (RECEIPT) → APPLYING and `applied` (APPLYING) → removed (C9.7.7 L3678) are **S6's edges** under Decision 11; S5 writes rows into RECEIPT, recovers rows found in either phase, and takes neither edge. `reconstruct` → the C9.7.11 L3793 ordinary reconstruction, which C9.7.6 L3628 counts as `recovered` under the **same** recovery key (C9.7.6 L3613, which contains no `action`), so it is a named replacement of that key's candidate and not a phase mutation of a live row — the Immutability rule below is not in play, because no member of a surviving row is rewritten. C9.7.7 L3674 forbids a cancel candidate reconstructing as one. `resume` and `replace` are C9.7.11 L3793's exact-epoch resume and C9.7.7 L3675's one-for-one lineage replacement, exactly as those rules state them; **this section deliberately does not restate either, and the resumed action remains a registered open decision** — see the SERVER_SNAPSHOT members rule for why the owner's stated mechanism does not close it. **Cancel.** `preflight` (none) → creates PREFLIGHT under C9.7.7 L3679's predicate only. `prepare` (PREFLIGHT) → PREPARED. `dispatch`, `receipt_mismatch`, `apply` and `applied` behave exactly as ordinary, with removal after the APPLYING obligation completes (C9.7.7 L3678). `retire` (**PREFLIGHT, PREPARED only**) → the C9.7.9 L3750 conclusive-no-commit retirement, lawful only in the same envelope write that creates its replacement, and never from RECEIPT or APPLYING because those bytes are never dropped (C9.7.6 L3638, L3640). **Both.** `purge` (any phase) → removed, only under the C2.2 account-deletion purge and the C2.4 auth-transition purge |
| Durable dispatch boundary | PREPARED is the pre-dispatch durable phase and is the store's proof that no call was issued. The transition to DISPATCHED is durably committed **before** the callable is invoked — that is what makes DISPATCHED the C9.7.8 pre-replay phase, and what stops a crash from leaving a PREPARED row behind a server mutation that in fact committed. Persisting DISPATCHED after the invocation would destroy PREPARED's no-dispatch meaning and is forbidden. No existing contract line states this boundary for the handoff families: C9.7.6 L3647 scopes its "no mutating server call" to *recovered target* durability and readiness, and C9.7.13 L3877 names handoff's operations only as "handoff operations" where L3880 breaks workflow's into `prepare`/`dispatch`/`application`. This rule is therefore **new authority created by S5-CD9**, stated because C9.7.6 L3629's "PREPARED ordinary handoff named no-dispatch proof" and C9.7.11 L3816's no-commit drop both read a phase as a dispatch fact and are unsound without it |
| Transport failure | A transport error, a malformed or noncanonical response, auth drift or digest drift writes nothing and leaves the row byte-identical in its current phase (C9.7.4 L3571). `pending` is unreachable for both handoff families, active RESET being its sole member (C9.7.8 L3722) |
| Application failure | A failing APPLYING obligation is re-run on each later readiness without a phase change and without a second server call (C9.7.6 L3640), for as long as it keeps failing. The contract states no retry bound and this store adds none: the row is never removed, never retired, and never inferred applied from server terminal state (C9.7.6 L3638, L3640). What the application *does* — its per-action effects, its completion predicate and its crash-safe acknowledgement — **is S6's, and lands with `functions/taskDisposition.js` derived from the real response shapes** (S5-CD10, owner ruling Decision 11). `applicationId` names the obligation and does not define it. S5 writes the store, the dispatch and the recovery conformance, and recovers RECEIPT and APPLYING rows byte-for-byte as C9.7.6 L3640 requires; it never performs an application, so the `apply` and `applied` edges below are S6's to take. P1-H and A3-5 therefore close at **S6**, not S5 |
| Reason pairing (C9.7.9 L3752) | `AUTH_EPOCH_CHANGED` requires `storedNamespace.installationId == requesterNamespace.installationId` and `storedNamespace.authEpochUUID != requesterNamespace.authEpochUUID`. `TAKEOVER` requires `storedNamespace.installationId != requesterNamespace.installationId`. Any other pairing is structural corruption |
| Requester namespace | Exactly `{installationId, authEpochUUID}`; preserved byte-for-byte across the row's valid phase (C9.7.9 L3751). The stored namespace names the ordinary session being cancelled and has the same two members. Neither carries a UID or a credential revision |
| Lineage | The C9.7.7 cancellation lineage identity `{uid,taskDocumentId,taskInstanceId,sessionId}` is computed from members both record types carry. A lineage is a **group**, not a pair: it holds every ordinary row and every cancel row sharing those four members. Because the ordinary recovery key adds `installationId` and `authEpochUUID` while the lineage identity does not, one lineage may lawfully hold several ordinary rows in different installation or epoch namespaces, so no rule may assume a unique partner. Replacement remains **per candidate** exactly as C9.7.9 L3749 and C9.7.7 L3673 state it ("replaces that one source candidate", "counts once"); this rule adds no group-wise collapse, and C9.7.11 L3807/L3808's Continue and Restart collapses are the only group operations, on their own authority. C9.7.7 L3680's projection is lineage-aware for the same grouping reason. Per-candidate replacement of two lawful ordinary rows that differ only in a namespace member now yields two **distinct** cancel keys, because S5-CD10 put `storedNamespace.installationId` and `storedNamespace.authEpochUUID` into the C9.7.6 L3614 key; before that they collided and the uniqueness rule called two required rows unresolved |
| Sort | Each array is ordered by `createdAt`, then by its complete C9.7.6 recovery key in the key's own member order, comparing every string member in unsigned UTF-8: ordinary by `uid`, `installationId`, `authEpochUUID`, `taskInstanceId`, `sessionId`; cancel by `uid`, `taskInstanceId`, `sessionId`, `reasonCode`, `requesterNamespace.installationId`, `requesterNamespace.authEpochUUID`, `storedNamespace.installationId`, `storedNamespace.authEpochUUID` (the last two added by S5-CD10 with the key itself). Because the key is unique within the array, the order is total. This is the frozen store order C9.7.6 L3645, C9.7.7 L3671 and C9.7.3 L3540 require; a noncanonical order is structural corruption. It is the **durable serialization order only** and never decides which mismatch is exposed: C9.7.8 L3703's exposed mismatch is the unsigned-UTF8-smallest `TaskCanonicalV1` encoding among actionable current-auth mismatches, computed from the identity maps and never from array position |
| Uniqueness | The C9.7.6 recovery key is unique within each array; a duplicate key in one source is unresolved, never dropped |
| Counts | Normal regime: `sessions.records` at most 16 and `reconciliationCancels` at most 16 (C9.7.7 L3662). The expanded regime's 32 / 96 / 64 / 128 ceilings (C9.7.7 L3655–L3657) and its 524,288 per-record bound (C9.7.7 L3659) apply only under its exact predicates; the normal regime states no separate per-record bound, and none is needed, because the normal envelope cap is itself 524,288 (C9.7.1 L3463) and the C9.7.1 L3456 `base + 190` reserve leaves any single record strictly less room than that bound |
| Opacity | No record carries a raw Keychain byte or `OSStatus`, a validated-provenance entry (C9.7.8 L3691 keeps that process-local), or a foreign source's installation singleton (C9.7.11 L3813). The three opaque strings — `requestCanonicalJSON`, `receipt`, `serverSnapshot` — are retained and hashed but never decoded by this store; every presentation value, `actionLabel` included, is read from the live server document at presentation time (C9.7.11 L3796), never from this file |

#### C9.7.19 Workflow durable store (S5-CD2)

```text
file    = PeezyWorkflowRequests-v2.json      (Application Support; protection completeUntilFirstUserAuthentication)
quarantine sibling = PeezyWorkflowRequests-v2.quarantine-v1.json
DurableFileEnvelopeV1 = {schemaVersion:1,fileKind:"WORKFLOW_REQUESTS_V2",generationId,payload:WorkflowPayloadV1,recoveryReceipt?,sha256}
WorkflowPayloadV1 = {records:[WorkflowRequestRecordV1],retiredDiagnostics:[WorkflowRetiredDiagnosticV1]}
   exact two-member set; both required; the empty state is {records:[],retiredDiagnostics:[]}

WorkflowRequestRecordV1
common   = {uid,authEpochUUID,credentialRevision,taskDocumentId,taskInstanceId,taskGenerationEpoch,
            workflowId,flowAttemptId,flowAttemptGeneration,submissionToken,phase,createdAt,updatedAt}
PREPARED   = common + {requestCanonicalJSON}
DISPATCHED = common + {requestCanonicalJSON}
RECEIPT    = common + {requestCanonicalJSON,receipt}
APPLYING   = common + {requestCanonicalJSON,receipt,applicationId}

WorkflowRetiredDiagnosticV1 = {schemaVersion:1,workflowId,taskInstanceId,flowAttemptGeneration,code,retiredAt}
   code = WORKFLOW_REQUEST_SUPERSEDED | WORKFLOW_REQUEST_ABANDONED   (closed set)
   retiredAt = the instant the **most recent** record with this element's tuple left `records`; never a creation instant. A coalescing write raises it, so it is a last-retirement stamp by construction and no earlier retirement instant is retained or recoverable

flowAttemptGeneration = safe nonnegative integer; a numeric member of the C9.7.6 recovery key (L3617, whose `taskGenerationEpoch` is numeric too) and of the C9.7.8 L3702 identity map. **This store never allocates it.** It is supplied by the flow attempt that prepares the row and is recorded as given: P1-B (L4938) puts the instance + attempt-generation + write-revision CAS in `FlowProgressSession.swift`, which is where the generation's monotonicity is enforced and proved. Reuse after a completed row is removed is not a collision here, because the server document is keyed on `{owner:uid,submissionToken}` alone (C9.7.8 L3713) and the identity map carries `submissionToken` and `flowAttemptId`, so a later attempt addresses a different `workflowSubmissionId`; within `records` the removed row is simply gone, and a colliding diagnostic tuple coalesces
requestCanonicalJSON  = UTF-8(TaskCanonicalV1(<the exact validated submitWorkflowAnswers request>)); an immutable opaque string, stored and hashed but never traversed
requestFingerprint    = recomputed from requestCanonicalJSON under the frozen workflow fingerprint rule (C9.7.8 L3719) and never stored
receipt               = UTF-8(TaskCanonicalV1(<the **exact stored response** C9.7.8 L3721 names for the workflow branch, without its outer `replayed:true` projection>)); an immutable opaque string, never decoded here. "Exact stored response" is C9.7.8's own term and is used here verbatim so the two rules cannot name different maps. The C9.7.8 receiptSHA256 is the lowercase SHA-256 of exactly these bytes, and hashes no outer callable wrapper, replay rewrite, response-authority expansion or omitted optional member (C9.7.8 L3689), so the digest has one preimage
applicationId         = "wap1_" + first40(SHA-256(TaskCanonicalV1({uid,identity_digest:<the row's C9.7.8 identityDigest>,receipt_sha256:<the row's C9.7.8 receiptSHA256>})))
createdAt, updatedAt  = canonical UTC RFC 3339 millisecond instants under the C9.5.4 `writeTime` rule
```

| Rule | Exact statement |
|---|---|
| Owner | The store is one process-wide serialized owner declared inside `WorkflowService.swift`: sole reader/modify/replacer of both files, one mutation at a time with atomic whole-value replacement. It is a separate durable-store type from the existing `WorkflowService` submission API, which keeps its present shape and call sites (C10.3: no submission-path change); no view, flow or callable path opens either file |
| Startup accept | Only exact outer/member sets, `fileKind` `WORKFLOW_REQUESTS_V2`, canonical bytes, valid generation UUID, valid payload, and recomputed `sha256` load; any other target is corrupt |
| Write protocol | Temp file in the same directory, POSIX write loop, `fsync` file, atomic `rename`, `fsync` directory |
| Corrupt target | Never reset empty; rename to the quarantine sibling, directory `fsync`, owner enters `CORRUPT_BLOCKED`; an existing quarantine leaves both files and stays blocked (P1-N). No Keychain authority gates this store |
| Capacity | One cap, 16,777,216 complete canonical outer-envelope bytes, with the C9.7.1 `base + 190` reserve and a physical read bound of cap + 1. The expanded regime is handoff's alone (C9.7.1 L3472). `records` holds at most 32 elements and each complete record is at most 524,288 bytes. A write that would exceed the envelope cap, the record count or the per-record bound writes nothing, leaves the target byte-identical, and returns the fixed local `WORKFLOW_STORE_CAPACITY_EXCEEDED` with zero server call and zero opener call. `retiredDiagnostics`' 16-element ceiling is **not** in that set: it is enforced by the pruning rule below, never by refusing the write, because a diagnostic exists only as the byproduct of a removal that has already been decided and refusing it would strand a PREPARED row that must leave `records` |
| PREPARED members | Requires `requestCanonicalJSON`; forbids receipt and application members. Durable before the dispatching await, on C9.7.13 L3880's authority that `prepare`, `dispatch` and `application` are three distinct workflow operations. C9.7.6 L3647 is **not** the basis and is not cited for it: it scopes its "no mutating server call" to *recovered target* durability and readiness and says nothing about a fresh row |
| DISPATCHED members | Same member set as PREPARED; the C9.7.8 pre-replay phase of the `WORKFLOW` family (L3713) |
| RECEIPT members | Requires the receipt; forbids `applicationId` |
| APPLYING members | Requires the receipt and `applicationId` |
| Immutability | Every `common` member except `phase` and `updatedAt`, plus `requestCanonicalJSON`, is immutable once written |
| Transitions — the in-operation event table | Every edge below is named by its **event** and its **source phase**. As in C9.7.18 this covers the store's own operation edges only: every recovery-time removal, drop and replacement is the named C9.7.3/C9.7.4/C9.7.6 action that produces it, on that rule's authority, and an edge in neither this table nor such a rule is structural corruption. `prepare` (none) → creates PREPARED, allocating and durably committing the attempt watermark in the same write. `dispatch` (PREPARED) → DISPATCHED, durably committed **before** the callable is invoked, exactly as C9.7.18's durable dispatch boundary requires and for the same reason. **`first response` (DISPATCHED) → RECEIPT**, the same-process response to the call this row issued, its provenance installed under C9.7.8 L3692's frozen first-response behavior and with no first-response notification, accounting or callback (C9.7.8 L3731); it is the only edge that stores a receipt outside recovery. `receipt_mismatch` `committed` (**RECEIPT, APPLYING only**) → the exact RECEIPT or APPLYING replacement of C9.7.4 L3571 with provenance installed. `receipt_mismatch` `absent` (**RECEIPT, APPLYING**) → DISPATCHED, the `WORKFLOW` pre-replay phase (C9.7.8 L3713), clearing only the members that phase forbids. The sources are the two receipt-bearing phases and never PREPARED or DISPATCHED, neither of which is receipt-bearing and so neither of which C9.7.3 L3540 can classify. `apply` (RECEIPT) → APPLYING and `applied` (APPLYING) → removed with no diagnostic are **S6's edges** under Decision 11, as in C9.7.18; S5 writes rows into RECEIPT and recovers rows found in either phase. `supersede` (**PREPARED only**) → removed with a `WORKFLOW_REQUEST_SUPERSEDED` diagnostic, when a later `flowAttemptGeneration` is prepared for the **same `(uid, taskInstanceId, taskGenerationEpoch, workflowId)`** — the recovery key less its generation. The `workflowId` qualifier is required: without it a second workflow started on the same task instance would evict the first workflow's PREPARED row and discard its answers, though L4148's uniqueness key exists precisely so two workflow IDs coexist. `abandon` (**PREPARED only**) → removed with a `WORKFLOW_REQUEST_ABANDONED` diagnostic, when the flow attempt is cleared. Both exits are lawful from PREPARED alone, because a PREPARED row proves no call was issued and owes no receipt; a DISPATCHED, RECEIPT or APPLYING row may **never** be superseded, abandoned or retired, its bytes being ones C9.7.6 L3638 never drops and L3640 always reruns. `purge` (any phase) → removed under the C2.2 and C2.4 purges only. `pending` is unreachable, active RESET being its sole member (C9.7.8 L3722), and a transport error, malformed response, auth drift or digest drift writes nothing (C9.7.4 L3571) |
| Application failure | Exactly as C9.7.18: re-run on each later readiness with no phase change and no second server call, for as long as it keeps failing, with no retry bound and no retirement. A failed application never removes the record and never converts it to a diagnostic — C9.7.6 L3638 never drops RECEIPT or APPLYING bytes and L3640 always reruns them, so retiring an unapplied obligation would discard a guarantee this amendment has no authority to change |
| Sort | `records` is ordered by `createdAt`, then unsigned-UTF-8 `uid`, then unsigned-UTF-8 `taskInstanceId`, then numeric `taskGenerationEpoch`, then unsigned-UTF-8 `workflowId`, then numeric `flowAttemptGeneration` — `createdAt` followed by the complete C9.7.6 L3617 recovery key in key order, so the order is total. `retiredDiagnostics` is ordered by `retiredAt`, then unsigned-UTF-8 `workflowId`, then unsigned-UTF-8 `taskInstanceId`, then numeric `flowAttemptGeneration`, then unsigned-UTF-8 `code`, total because the last four are unique within the array under the coalescing rule below. A noncanonical order is structural corruption. Both are the **durable serialization order** |
| Uniqueness | The C9.7.6 recovery key `(uid,taskInstanceId,taskGenerationEpoch,workflowId,flowAttemptGeneration)` is unique within `records`; a duplicate within one source is unresolved, never dropped. A `retiredDiagnostics` element is unique on `(workflowId,taskInstanceId,flowAttemptGeneration,code)` |
| Diagnostics | `retiredDiagnostics` elements are not candidates: they never enter `pendingRecordCount` (C9.7.6 L3624), a quarantine's are omitted on recovery and a live valid target's are carried byte-identically (C9.7.6 L3644). An element is appended only when a record leaves `records` from PREPARED without completing — superseded by a later attempt generation, or abandoned when its flow attempt is cleared — and carries no request bytes, no receipt, no answer content, and no account, path or provider identity, which is why no UID appears in its uniqueness key. Because it carries no UID, two accounts can retire the same tuple: a write whose uniqueness tuple already exists **coalesces**, updating that element's `retiredAt` to the later instant rather than appending, so the array stays deterministic under the frozen sort. At the 16-element ceiling the **first** element in the frozen sort order is dropped in the same write — pruning, never a capacity failure. Because a coalescing write raises `retiredAt` and moves its element to the end, the dropped element is always the least recently retired |
| Auth state | Each record carries its own `{uid,authEpochUUID,credentialRevision}`; there is no auth singleton, which is why C9.7.6 L3624 excludes only `retiredDiagnostics` and names no auth member. That per-record auth state is never dropped (C9.7.6 L3638) and a row is actionable only while signed in to its UID (C9.7.8 L3732) |
...
| spec | `TaskPlanService.swift` | S1 is sole writer and defines all typed callable DTOs/transports up front; S2 consumes them and may add tests only, as does S5 outside the three transports this row names, and as does S7 outside the one transport this row names for it → S3 adds `ResetOperationRegistry.drive`/`recoverEpoch` and the reset/inspection/reconciliation DTOs and transport (S3 brief decision) → S4 adds the `LegacyResetMigrationV1` rows inside `ResetOperationRegistry`, the `LEGACY_RESET_MIGRATION` inspection transport, and the `DurableStoreRecovering` conformance of `ResetOperationRegistry` over its existing `recoverySlot` (S4-CD5) → S5 adds the `HANDOFF`, `HANDOFF_CANCEL` and `WORKFLOW` `inspectCommittedOperation` transports only, because C9.7.8 resolves `receipt_mismatch` for `HandoffSessionStore` and `WorkflowService` through no other path (S5-CD4); → S7 adds the `ROUTE_CLAIM` `inspectCommittedOperation` transport only, with `AppRouteInbox`, on the same footing and for the same reason (C9.3.7's route conformer is S7's at C9.7.14 L3924, and C9.7.8 resolves its `receipt_mismatch` through no other path); nothing else (S4-CD2, S5-CD9) |
| S4-CD2 | `Peezy 4.0/Tasks/Durable/DurableStoreReadiness.swift` | S1 declares every seam → S4 adds the one-time runtime installation (`FirestoreRuntime.Registry`, `FirestoreRuntime.install(_:)`, `InstallOutcome`, `FirestoreRuntimeError`, replacing the `static let provider` with a registry-backed computed `provider` and removing `TransitionalFirestoreRuntime`, C9.7.16), the three missing `DurableFileKind` cases (`TASK_ROUTE_INBOX_V1`, `HANDOFF_AUTHORITY_V1`, `WORKFLOW_REQUESTS_V2`), the C9.7.2 `dose/malformed` `BlockedSnapshot` member with a `RecoveryStore = DurableStore | dose` projection for `store`/`busy`/`unavailable` (S4-CD1), `RecoveryObservedStateV1`, `RecoveryAttemptKey`, `RecoveryAction`, and the `DurableStoreRecovering` protocol (S4-CD5), and the `NarrationLease` value (S4-CD7); nothing else |
| S4-CD1 | `Peezy 4.0/MainInterface/Models/DailyDoseEngine.swift` | S1 owns the file (S3 client-reset-driver edit per the S3 brief) → S4 adds `DailyDoseLocalStore.observeMalformed(uid:)`, `quarantineMalformed(uid:expectedBytesSHA256:)`, and the dose recovery slot; nothing else |
| S4-CD5, S5-CD2 | `Peezy 4.0/MainInterface/Models/WorkflowService.swift` | S2 owns the file → S5 adds the `WorkflowAccountDeletionPurging` and `DurableStoreRecovering` conformances, and exactly the implementation those two conformances and their purge postconditions require: the C9.7.19 durable store (envelope read/write, atomic replacement, quarantine, the active-record grammar and its phases, the empty state, the frozen sort) and the C9.7.3 classification and C9.7.4 actions over its two files. S5 additionally records `prepare` and `dispatch` into that store on the submission path, because a `DurableStoreRecovering` conformance over a store nothing ever writes is vacuous — the narrowest widening the conformance needs, and no other submission-path change (S5-CD11, owner ruling Decision 10). Nothing else: no workflow product behavior, no callable edit, and no repair of the writer/inspector/client protocol, which is **S5B's** (S5-CD2) → S6 adds the local application of a RECEIPT row: the `apply` and `applied` edges of C9.7.19 and their per-action effects, completion predicate and crash-safe acknowledgement, with `functions/taskDisposition.js` (S5-CD10, Decision 11) |
| S5-CD10 | `Peezy 4.0/Tasks/Handoff/HandoffSessionStore.swift` | S5 creates the file and owns the store, the dispatch and the `DurableStoreRecovering` conformance, recovering RECEIPT and APPLYING rows byte-for-byte (C9.7.6 L3640) and never applying one → S6 adds the local application of a RECEIPT row: the C9.7.18 `apply` and `applied` edges, the per-action effects, completion predicate and crash-safe acknowledgement, and the three frozen `receipt`/`requestCanonicalJSON`/`serverSnapshot` ceilings C9.7.7 L3680's projection needs, all derived from the real response shapes with `functions/taskDisposition.js`; nothing else in the file. P1-H and A3-5 close at S6 (S5-CD10, owner ruling 2026-09-08, Decision 11) |
| S5-CD6 | `Peezy 4.0/MainInterface/Models/PeezyHomeViewModel.swift` | S4 owns the file (C10.2) → S5 routes the three `PeezyHomeViewModel` dose reads and writes through `DailyDoseLocalStore` under the C9.5.16 v2 epoch/revision CAS, replacing the `HomeDoseDefaults` accessors of `peezy.{uid}.dailyDose.completedCount`, `.lastDate` and `.firstLaunchDate`, and re-points the `PeezyNudgeAnswerTests` pinned assertions to the v2 state; nothing else. C9.5.16's Owner rule is thereby satisfied on the client, and the three v0 keys remain in the C6.6 registry and the preference barrier because the 0→1 bridge still reads them (S5-CD6) |
| spec | `PeezyMainContainer.swift` | S4 shared surface → S7 route follow-on |
| S5-CD7 | `Peezy 4.0/Assessment/AssessmentModels/AssessmentDataManager.swift`, and the single call site in `Peezy 4.0/Assessment/AssessmentModels/AssessmentCoordinator.swift` | S1 core/reset owns the data manager and S4 owns the coordinator (C10.2) → S5 changes `computeDistanceAndInterstate()` to **return** all three of its outputs — the distance category `moveDistance`, the interstate flag `isInterstate`, and the raw `moveDistanceMiles`, on both the success and the geocode-failure path — as one operation-local value instead of assigning them to the shared manager, and changes the one `completeAssessment()` line that awaits it to capture that value and assign the three fields only after the existing P1-R UID and load-generation guard. Nothing else in either file: no assessment question, storage, geocoding-provider, save-path or guard change, and no new test file. The falsifier is **not** the `AppRootAuthRaceTests` guard pin, which exercises only the `completionIsCurrent` predicate and passes with the defect present; it is the existing source pin `assessmentCompletionGuardsAfterEveryAwaitAndConditionsItsDeferredMutation` in `DurableStoreRecoveryTests.swift`, which today requires the literal `_ = await geocodeTask.value`. Because capturing the result changes that literal, S5 updates exactly that pin — inside the file the S4-CD5 row already opens to it and nowhere else in it — to require the capture, the guard between the capture and the first assignment, and that no assignment to any of the three fields appears before the guard. S1's owner reviews the data-manager diff and S4's the coordinator line and the pin at S5 close-out (S5-CD7, corrected S5-CD9) |
...
| S4-CD3 | `Peezy 4.0Tests/TaskSupersessionTests.swift` | S2 owns the file (C10.4) → S4 contributes the D18–D21 cases (C9.5.20–C9.5.24) and re-runs the class; S6 contributes its adapter cases later; S2's cases are never edited |
...
| manifest, S4-CD6, S4-CD7 | `Peezy 4.0/Inventory/Models/InventorySessionManager.swift` | S1 migrates exactly its six `Firestore.firestore()` occurrences to `FirestoreRuntime` (the S4-CD2 installation makes `FirestoreRuntimeOwner` the published provider without touching them) → S4 changes `pendingNarration` to hold actor-issued lease handles, adds the §8.9.3 admission/generation check (current signed UID, `clear` gate, matching `FirestoreRuntimeGeneration`; a held response across a generation change is discarded, never applied) at the existing `InventoryAPIClient` call sites, and wraps the existing `storageService.uploadFrames` call with `RoomCaptureArtifactOwner.register(transfer:cancel:)` / `settle(_:)` (S4-CD7); no other edit is authorized |
...
| spec, manifest, S4-CD4 | `Peezy 4.0Tests/AppRootAuthRaceTests.swift` | S4 owns the class and the auth-race families against seams → S5 contributes the Google callback-slot fixtures, the sole-`GIDSignIn.sharedInstance` call-site scan, the sole-`UNUserNotificationCenter`/`Messaging` deletion-call-site scan, the pinned Messaging/Installations API-order fixture, and the `[fakes, realConformers]` parameterization of the families the S4-CD5 row names in this class and nothing more of it (S5-CD9) → S7 contributes the eleven runtime `ObjectIdentifier` identities and the mount cases → S4 performs the final full-class review |
| S4-CD5 | C2.7 two-device crash-mid-purge and A→B families (`DurableStoreRecoveryTests.swift`, `AppRootAuthRaceTests.swift`) | S4 proves them on the C9.7.14 seam fakes → S5's close-out re-runs the same families on the real conformers (`GoogleIdentityAuthority`, `NotificationIdentityAuthority`, `HandoffSessionStore`, `WorkflowService`) unchanged → S5 adds to `DurableStoreRecoveryTests.swift`, and to nothing else in it, exactly the real-owner construction those reruns need (S5-CD5): a `RealConformerHarness` beside the existing `makeDeletionHarness` that builds `LocalPurgeOwners` and `DurableStoreOwners` from the four real conformers over a temporary directory, an isolated `UserDefaults` suite and the emulator, plus a `GateSnapshotStub`-equivalent gate and the S5 authorities' own injected seams. The families are named exactly: in `DurableStoreRecoveryTests.swift` the C2.7 two-device crash-mid-purge family and the A→B purge family; in `AppRootAuthRaceTests.swift` the seven `makeDeletionHarness` call sites' auth-race and A→B families. Because `DeletionHarness.owners` is the concrete `RecordingPurgeOwners`, whose logging and failure controls the vectors assert on, substitution needs one common observation and fault-injection adapter: S5 adds a `PurgeObservation` protocol that `RecordingPurgeOwners` and the real-conformer harness both satisfy, `DeletionHarness.owners` becomes that existential, and no vector or expectation changes. Every vector, expectation and test name in the C2.7 families is unchanged; only the owners passed in are substituted, and each named family in **both** classes gains one parameterization over `[fakes, realConformers]` rather than a copied case — the parameterization of the `AppRootAuthRaceTests` families being the only S5 edit that row authorizes for this purpose. Each parameterization must be observably real: the `realConformers` position fails if the real owner is not the object the coordinator called. S4 remains the class owner of both files and performs the final full-class review of each (S5-CD5, corrected S5-CD9) |
| S4 brief Decision 7 (ledger exception) | `Peezy 4.0/Menu/PeezySettingsView.swift` `deleteAccount()` hunk (C9.4.6) | S1 owns the file → S7 replaces the hunk under the C7 byte guard when it declares `Phase2ProductionRuntime.accountDeletionCoordinator`; S4 delivers the coordinator type and its production initializer; no S1 follow-on |
...
| manifest | `Peezy 4.0Tests/DurableStoreRecoveryTests.swift` | `UID-interpolated preference keys are registry-complete` | a source scan failing on any UID-interpolated `UserDefaults` or `CFPreferences` key outside the eleven-key registry |
...
| spec, S5-CD8 | `PEEZY_STATE.md` | unchanged except §4, and within §4 only the artifact versions and hashes: the executable-authority sha256 line, the amendment lineage it carries, and one head-hash line per closed slice, updated at each amendment registration and each slice close. Every other part of the file, including §§1–3 and §4's resolution sign-off trail, stays regeneration-only, and no slice narrative is written there (S5-CD8) |
...
| manifest | `firebase.json`, `functions/taskCatalogData.json`, `functions/flowDefinitionsData.json`, `public/admin/index.html`, the Xcode project, active catalog/flow data, router shadows, live config | remain excluded |
| manifest, S5-CD8 | `PEEZY_STATE.md` | remains excluded except §4 under the C10.5 spec row above (S5-CD8) |
...
| S4-CD1 | S4 brief (Decision 5; brief gate BLOCKER 1; owner ruling 2026-09-06) | C9.7.2 gains the `dose/malformed` member, C9.7.4 the non-destructive `quarantine_dose_bytes` action, C9.7.12 the `dose_quarantine` attempt key, C9.7.13 the rule that a malformed dose store blocks only the reset's dose cleanup; legacy keys are never touched. | `Peezy 4.0/Tasks/Durable/DurableStoreRecoveryCoordinator.swift`, `DurableStoreRecoveryView.swift`, `DailyDoseEngine.swift` (`quarantineMalformed`); `Peezy 4.0Tests/DurableStoreRecoveryTests.swift` dose classification/quarantine/crash cases and the eleven-key registry scan |
| S4-CD2 | S4 brief (Decisions 1–2; brief gate BLOCKER 6) | C10.3 writer sequences for `TaskPlanService.swift` (S1 → S3 → S4: `LegacyResetMigrationV1` rows and the `LEGACY_RESET_MIGRATION` transport only) and `DurableStoreReadiness.swift` (S1 → S4: one-time installation and three `DurableFileKind` cases only); C9.7.16 states `FirestoreRuntime.install(_:)`. | `TaskPlanService.swift`, `DurableStoreReadiness.swift`; `DurableStoreRecoveryTests.swift` migration rows and install-once cases |
| S4-CD3 | S4 brief (Decision 3; brief gate BLOCKER 7) | C9.5.24 defines `TaskDispositionSurfaceState`; D18–D21 production and falsifiers are S4's; C10.3 row for S4's `TaskSupersessionTests` contributions; `PEEZY_STATE.md` §4 corrected. | `Peezy 4.0/Tasks/Disposition/TaskDispositionSurface.swift`; `TaskDispositionSurfaceTests.swift`; `TaskSupersessionTests.swift` D18–D21 cases (S4 contribution) |
| S4-CD4 | S4 brief (Decision 6; brief gate HIGH 9) | C10.3 orders the `AppRootAuthRaceTests` contributors S4 → S5 → S7 → S4 final review, naming S7's eleven-identity and mount cases. | `Peezy 4.0Tests/AppRootAuthRaceTests.swift` |
| S4-CD5 | S4 brief (Decisions 10–11; brief gate BLOCKER 2) | C9.7.14 gains `DurableStoreRecovering` (observe/classify/perform; slot in the owner) with the four conformers, names `WorkflowService` the workflow store owner with S5 writing its `WorkflowAccountDeletionPurging` conformance; C10.3 row requires S5's close-out to re-run the C2.7 two-device and A→B families on the real conformers. | `DurableStoreReadiness.swift` (declarations, S4-CD2 scope), `DurableStoreRecoveryCoordinator.swift`, `WorkflowService.swift` (S5 conformances); `DurableStoreRecoveryTests.swift` coordinator-through-protocol cases |
| S4-CD6 | S4 brief (Decision 12; brief gate BLOCKER 3) | C10.3 L4170 authorizes the §8.9.3 admission/generation check at the existing `InventoryAPIClient` call sites in `InventorySessionManager.swift`; no other edit. | `InventorySessionManager.swift`; `DurableStoreRecoveryTests.swift` held-response/generation cases |
| S4-CD7 | S4 brief (Decision 13; brief gate HIGH 8) | C9.7.14 gains `RoomCaptureArtifactOwner` and the `NarrationLease` shape: acquire only under a clear gate for the UID and generation, post-await revalidation, `revokeAll()` before the room_capture ack with cancellation completing first. | `LocalPrivacyPurgeCoordinator.swift`, `InventorySessionManager.swift`, `InventoryCameraView.swift`, `NarrationService.swift`, `InventoryItemConfirmView.swift` (media-transfer completions); `DurableStoreRecoveryTests.swift` narration/media lease cases |
...
| S5-CD1 | S5 brief gate (BLOCKER 2, run on Sol before the reviewer change; owner ruling 2026-09-08, Decision 1) | C9.7.18 states the handoff record grammar the contract had left to a one-line summary: both record unions (`HandoffSessionRecordV1`, `ReconciliationCancelV1`), every phase's required and forbidden members, the transition table, the frozen sort, and the request/receipt mapping, drafted from the C9.4.5 legacy-migration and C9.3.7 route-record precedents. | `Peezy 4.0/Tasks/Handoff/HandoffSession.swift`, `Peezy 4.0/Tasks/Handoff/HandoffSessionStore.swift`; `Peezy 4.0Tests/HandoffSessionTests.swift` grammar cross-product and both recovery keys |
| S5-CD2 | S5 brief gate (BLOCKER 3; owner ruling 2026-09-08, Decision 2) | C9.7.19 states the workflow durable store the contract had fixed only by `fileKind`, cap and counted universe: its target and quarantine names, protection class, payload, active-record grammar and phases, empty state and frozen sort; the C10.3 `WorkflowService` row is widened from "the two conformances and nothing else" to exactly the implementation those conformances and their purge postconditions require, and no more. | `Peezy 4.0/MainInterface/Models/WorkflowService.swift`; `Peezy 4.0Tests/HandoffSessionTests.swift` and `ConversationFlowTests.swift` workflow-store cases |
| S5-CD3 | S5 brief gate (Decision 6 of the brief; owner ruling 2026-09-08, Decision 3) | C9.7.17 fixes the handoff store's durable file names in the C9.3.6 shape — `PeezyHandoffAuthority-v1.json` and `PeezyHandoffAuthority-v1.quarantine-v1.json`, Application Support, protection `completeUntilFirstUserAuthentication` — which the contract had left unnamed while naming the route store's. | `Peezy 4.0/Tasks/Handoff/HandoffSessionStore.swift`; `HandoffSessionTests.swift` crash/tamper/quarantine suite |
| S5-CD4 | S5 brief (Decision 3; owner ruling 2026-09-08, Decision 4) | C10.3 extends the `TaskPlanService.swift` writer sequence with S5 for the `HANDOFF`, `HANDOFF_CANCEL` and `WORKFLOW` `inspectCommittedOperation` transports only, because C9.7.8 resolves `receipt_mismatch` for `HandoffSessionStore` and `WorkflowService` through no other path; `ROUTE_CLAIM` stays S7's. | `Peezy 4.0/MainInterface/Models/TaskPlanService.swift`; `HandoffSessionTests.swift` receipt-mismatch reconcile cases |
| S5-CD5 | S5 brief gate (BLOCKER 7; owner ruling 2026-09-08, Decision 5) | C10.3 authorizes S5's exact-scope contribution to `DurableStoreRecoveryTests.swift`: a real-owner harness beside the existing seam-fake one, with every C2.7 vector, expectation and test name unchanged and only the injected owners substituted. | `Peezy 4.0Tests/DurableStoreRecoveryTests.swift` C2.7 two-device and A→B families parameterized over fakes and real conformers |
| S5-CD6 | S4 close-out review (finding 9; S5 brief Decision 1; owner ruling 2026-09-08, Decision 6) | C10.3 adds the ordered-writer row for `PeezyHomeViewModel.swift`: S4 owns the file → S5 routes the three dose reads and writes through `DailyDoseLocalStore` under the C9.5.16 v2 epoch/revision CAS and nothing else, satisfying C9.5.16's Owner rule on the client. | `Peezy 4.0/MainInterface/Models/PeezyHomeViewModel.swift`; `Peezy 4.0Tests/PeezyNudgeAnswerTests.swift` assertions re-pointed to the v2 state |
| S5-CD7 | S4 close-out review (finding 20; S5 brief Decision 2; owner ruling 2026-09-08, Decision 7) | C10.3 adds the ordered-writer row for `AssessmentDataManager.swift` and the single `AssessmentCoordinator.completeAssessment()` call site: S1 owns the data manager and S4 the coordinator → S5 makes `computeDistanceAndInterstate()` return an operation-local value the caller applies only after its P1-R guard, and nothing else, each file's owner reviewing its diff. | `Peezy 4.0/Assessment/AssessmentModels/AssessmentDataManager.swift`, `AssessmentCoordinator.swift`; no new test — S4's existing `AppRootAuthRaceTests` guard pin, whose premise this makes true |
| S5-CD8 | S5 brief gate (BLOCKER 8; owner ruling 2026-09-08, Decision 8) | C10.5 excludes `PEEZY_STATE.md` except §4, which becomes the single authority for artifact versions and hashes and is updated at each amendment registration and each slice close; all slice narrative belongs to `STATUS.md` and the ledger. | `PEEZY_STATE.md` §4; `STATUS.md`; `tasks/todo.md` |
| S5-CD11 | Owner ruling 2026-09-08 on Decision 10 (the split) | C10.3 widens S5's `WorkflowService.swift` row so the store may record `prepare` and `dispatch` on the submission path — the narrowest widening its `DurableStoreRecovering` conformance needs, since a recovery conformance over a store nothing ever writes is vacuous — and no other submission-path change. The writer, inspector and client-submission protocol repair is **not** S5's: it becomes slice **S5B**, with its own brief, its own gate and a real-writer → crash → inspector → local-application emulator falsifier, running after S5 and before S6. It is not a S7 pre-ship gate item, that list being actions the owner performs and not a two-runtime protocol repair. **P1-N and C9.7.14 L3924 close at S5B, not S5.** The protocol statement itself lands as its own amendment before I11: the writer's actual on-disk format is authoritative wherever production documents already exist — document id prefix, fingerprint prefix and preimage, field casing, token version — and the inspector is repaired to match it; the writer is extended only where it emits nothing today, the six identity fields; and any point with no production data follows the form of the two specified siblings, `"reset1_" + SHA-256(TaskCanonicalV1(...))` and `"rlmreq1_" + SHA-256(TaskCanonicalV1(...))`. | C10.3 `WorkflowService.swift`; C9.7.8 L3713 and L3719 at the protocol amendment; new slice S5B |
| S5-CD10 | Owner rulings 2026-09-08 on Decisions 11 and 12 | **Decision 11:** the handoff and workflow **application** half moves to S6 — its per-action effects, completion predicate and crash-safe acknowledgement, and the three per-member ceilings C9.7.7 L3680's projection needs, all derived from the real response shapes with `functions/taskDisposition.js`. S5 keeps the store, the dispatch and the recovery conformance, recovers RECEIPT and APPLYING rows byte-for-byte, and takes neither the `apply` nor the `applied` edge. No ceiling is invented: a conservative one makes the normal regime unreachable, and inventing a tighter one is the failure the S4 review named. **P1-H and A3-5 close at S6, not S5.** **Decision 12:** the SERVER_SNAPSHOT resume is nameable — the snapshot's own C9.7.6 L3613 key *is* the identity key of the row it replaced (C9.7.6 L3628 counts the reconstruction as `recovered` under that key), it carries no `action`, and the resumed action is read from the referenced row, meaning the live server session document under that key at C9.7.11 L3796's presentation-time read; no separate reference member is stored, because a duplicate of members `common` already carries could disagree with them. And the C9.7.6 L3614 cancel recovery key gains `storedNamespace.installationId` and `storedNamespace.authEpochUUID`, so the two PREFLIGHTs that C9.7.9 L3749's per-candidate replacement produces from two lawful ordinary rows can no longer collide. | C9.7.6 L3614; C9.7.18; C9.7.19; C10.3 `HandoffSessionStore.swift` and `WorkflowService.swift` |
| S5-CD9 | S5 brief re-gate, **GPT-6 Astra** round 2 (4 BLOCKER + 4 HIGH), then two fresh-context passes over the correction itself (24 further defects, 7 of them blocking) | Corrects S5-CD1, CD2, CD4, CD5 and CD7 where the registered text was internally inconsistent, contradicted an uncited row, or was not executable. It changes no approved row's meaning, and where it could not fix a gap it **narrows the amendment's claim rather than inventing**. **The happy path was missing.** No edge stored a receipt outside recovery, so RECEIPT and APPLYING were unreachable, DISPATCHED was absorbing, and `apply`, `applied`, `applicationId` and the whole Application-failure rule were dead text; `first response` (DISPATCHED) → RECEIPT is added to both stores under C9.7.8 L3692's frozen first-response behavior and L3731's no-notification rule. **The exhaustiveness claim was false and branded required paths corruption.** A phase-indexed table had forbidden the edge C9.7.8 L3730 requires; the tables are now **event × source phase**, and their scope is narrowed to the store's own operation edges, because every recovery-time removal, drop, replacement and reconstruction is the named C9.7.4 action, C9.7.11 choice or C9.7.9 replacement that produces it — C9.7.4 L3569's "never removed/changed except named reconciliation replacement" makes the named action the instrument, and C9.7.11 L3789, C9.7.6 L3629, C9.7.11 L3807/L3808 and C9.7.9 L3749 are all lawful edges an earlier draft called corruption. `receipt_mismatch`'s sources are the two receipt-bearing phases only, never DISPATCHED, which C9.7.3 L3540 cannot classify. **Two things were invented authority and are removed:** a "frozen retry bound" that appears nowhere, and a `WORKFLOW_APPLICATION_BLOCKED` retirement that would have deleted the only receipt-bearing record without completing its application, against C9.7.6 L3638 and L3640. So is a third: `attemptWatermarks`, added in an earlier correction to allocate `flowAttemptGeneration`, is **deleted** — its premise was false (C9.7.8 L3713 keys the server document on `{owner:uid,submissionToken}` alone, so a reused generation addresses a different document) and it wedged the store permanently once 32 instances completed under one epoch. The generation is supplied by the flow attempt, where P1-B's CAS enforces it, and this store records it. Also: one canonical receipt representation with one hash preimage, in C9.7.8 L3721's own words "exact stored response"; a durable dispatch boundary, disclosed as **new authority created here** because no existing line states it for the handoff families and C9.7.6 L3629 and C9.7.11 L3816 both read a phase as a dispatch fact; DISPATCHED restated as "may have been issued"; the validation obligation stated in terms the store can meet without decoding an opaque string, with `{requestSHA256}` raw and no `taskDocumentId` identity member; the frozen sort declared a serialization order; lineage a group for projection but replacement left per-candidate on C9.7.9 L3749's authority; `supersede` qualified on `workflowId`; the diagnostic ceiling pruned rather than refused; `retiredAt` defined as a last-retirement stamp; and five mis-citations fixed (C9.6.6 L3200 for C9.6.7, C9.7.7 L3655–L3657 for the four ceilings, C9.7.13 L3880 for workflow's three operations, and C9.7.6 L3647 removed as a basis it never supported). C10.3: S7's `ROUTE_CLAIM` an exact transport per the owner's "ROUTE_CLAIM stays S7's"; the CD5 harness naming its families in both classes with the one observation adapter the concrete `RecordingPurgeOwners` forces; CD7 returning all three geocode outputs and naming its real falsifier, the `DurableStoreRecoveryTests` source pin requiring the literal `_ = await geocodeTask.value`, in place of the `AppRootAuthRaceTests` predicate pin that passes with the defect present. Three gaps are left **explicitly open and registered rather than invented**: the resumed action after a SERVER_SNAPSHOT, which no rule names and no member carries; the workflow request fingerprint, which C9.7.8 L3719 never defines; and the identical cancel key two per-candidate replacements can produce. | C9.7.18; C9.7.19; C10.3 `TaskPlanService.swift`, `AppRootAuthRaceTests.swift`, the S4-CD5 harness row and the S5-CD7 row |

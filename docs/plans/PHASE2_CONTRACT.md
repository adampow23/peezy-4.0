...
| spec | `TaskPlanService.swift` | S1 is sole writer and defines all typed callable DTOs/transports up front; S2/S5/S7 consume them and may add tests only → S3 adds `ResetOperationRegistry.drive`/`recoverEpoch` and the reset/inspection/reconciliation DTOs and transport (S3 brief decision) → S4 adds the `LegacyResetMigrationV1` rows inside `ResetOperationRegistry`, the `LEGACY_RESET_MIGRATION` inspection transport, and the `DurableStoreRecovering` conformance of `ResetOperationRegistry` over its existing `recoverySlot` (S4-CD5); nothing else (S4-CD2) |
...
| S4-CD5 | `Peezy 4.0/MainInterface/Models/WorkflowService.swift` | S2 owns the file → S5 adds the `WorkflowAccountDeletionPurging` and `DurableStoreRecovering` conformances and nothing else |

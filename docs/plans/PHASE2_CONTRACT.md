...
TaskDispositionSurfaceState =
 ...
 | readOnly(contract, reason)                    // reason: "completed"|"dismissed"|"legacy_read_only"|"gate_nonclear"|"store_blocked"
 | superseded(SupersededContractV2)              // decoded by the C9.5.20 decoder; rendered per C9.5.21/C9.5.23 as `Replaced — {date}`
```
...
| Superseded | A valid v2 superseded record (C9.5.20) is `superseded`; a malformed record is `readOnly(_, "legacy_read_only")` with the C9.5.20 `malformedPresent` behavior; never `actionable` |
...
| `dose/malformed` (S4-CD1) | Produced only for the current signed UID by `DailyDoseLocalStore.load(uid:) == .malformed` (C9.5.16); `bytesSHA256`/`byteLength` are the malformed v2 bytes' digest and length; no legacy-key value, no payload byte, no path enters the snapshot; it blocks only the reset's dose cleanup (C9.7.13), never reserve/bind/dispatch |
...
| `dose/malformed` (S4-CD1) | `quarantine_dose_bytes` | `RecoveryAttemptKey{kind:"dose_quarantine",recoveryStateDigest}`; digest match pre-call and post-await; the v2 bytes reread and SHA-256 equal to the snapshot's `bytesSHA256` immediately before the write | Copy the malformed v2 bytes byte-for-byte to `peezy.{uid}.dailyDose.v2.quarantine` (one `UserDefaults` set), then remove `peezy.{uid}.dailyDose.v2` (one remove), then `CFPreferencesAppSynchronize`; the three legacy keys are never read, written, or removed; the next dose read finds v2 absent and the C9.5.16 bridge/floor rules apply; the quarantine key is removed only by account deletion (C2.2 preference barrier: it is the eleventh C6.6 UID-scoped key); the action is performed by `DailyDoseLocalStore.quarantineMalformed(uid:expectedBytesSHA256:)` (the sole v2-key actor, C6.6), which holds the dose `(RecoveryAttemptKey,Task)` slot; the coordinator invokes it and never touches the key | Crash after the copy and before the remove leaves both keys: reclassification sees the same malformed v2 bytes and the same digest and repeats; crash after the remove leaves the quarantine key only: the store is ready |
...
| `RoomCaptureArtifactOwner` / `NarrationLease` (S4-CD7) | `NarrationLease = {leaseId (lowercase UUID), uid, gateGeneration, sessionId}`; `RoomCaptureArtifactOwner` (S4, inside `LocalPrivacyPurgeCoordinator.swift`) is the sole issuer: `acquire(uid:sessionId:) async -> NarrationLease?` returns a lease only while the deletion gate is `clear` for that UID and the current `GateGeneration`, else nil; `revalidate(_ lease:) -> Bool` is true only while the lease is outstanding and the gate generation is unchanged; `release(_ lease:)`; `revokeAll()` marks every outstanding lease invalid and awaits every in-flight transfer's cancellation before returning; `NarrationService.start(lease:)`, `pendingNarration`, and `pendingNarrationTranscript` hold only a `NarrationLease`; every transfer completion and every narration materialization revalidates the lease after each await and drops its buffer on `false`; gate activation calls `revokeAll()` before the room_capture ack (C2.2 order), so cancellation completes before revocation returns and nothing materializes after the ack |
...
FirestoreRuntime.install(_ owner: FirestoreRuntimeOwner) -> InstallOutcome (S4-CD2): `installed` exactly once per registry and only before any lease is issued; a second call returns the fixed `alreadyInstalled`, a call after a lease the fixed `leaseIssued`, each with zero state change (no replacement, no second runtime); before installation `published()`/`acquire()`/`firestore()` are refused with the fixed `notInstalled` (no default instance); `FirestoreRuntime.shared` is the one process registry and `Phase2ProductionRuntime.firestoreRuntimeOwner` its sole production caller; tests drive install-once through an injected `FirestoreRuntime.Registry` instance, one per test
...
| manifest, S4-CD6 | `Peezy 4.0/Inventory/Models/InventorySessionManager.swift` | S1 migrates exactly its six `Firestore.firestore()` occurrences to `FirestoreRuntime` (the S4-CD2 installation makes `FirestoreRuntimeOwner` the published provider without touching them) → S4 changes `pendingNarration` to hold actor-issued lease handles and adds the §8.9.3 admission/generation check (current signed UID, `clear` gate, matching `FirestoreRuntimeGeneration`; a held response across a generation change is discarded, never applied) at the existing `InventoryAPIClient` call sites; no other edit is authorized |

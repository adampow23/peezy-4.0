...
// MARK: - Reserve Elevators Old Flow
...
struct ReserveElevatorsOldFlow: View {
    let taskTitle = "Reserve my loading elevator"
    let workflowId = "reserve_elevators_old"

    let userId: String
    let currentAddress: String  // currentAddress from UserState (origin)
    let moveDate: Date
    let onComplete: () -> Void
    let onDismiss: () -> Void
    ...
    private var cardContent: some View {
        ...
        case confirmAddressCard:
            TaskFlowConfirmAddressCard(
                taskTitle: taskTitle,
                question: "Is this where you're loading from?",
        ...
        case tipCard:
            TaskFlowInfoCard(
                taskTitle: taskTitle,
                title: "Good to Know",
                bodyText: "Measure your couch against the elevator BEFORE move day.",
                primaryLabel: "Got it",
                showBack: true,
                onPrimary: { advance() },
                onBack: { goBack() }
...
#if DEBUG
#Preview("Reserve Elevators Old Flow") {
    ReserveElevatorsOldFlow(
        userId: "preview-user",
        currentAddress: "1842 Oak Park Ave, Kansas City, MO 64108",

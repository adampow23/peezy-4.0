...
struct FinancialInstitutions: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            GeometryReader { geo in
                VStack(spacing: 0) {
                    ...
                    // Add entry field
                    HStack(spacing: 12) {
                        TextField("", text: $currentEntry, prompt: Text("e.g. Chase, Amex, Fidelity").foregroundColor(.white.opacity(0.3)))
                            .font(.system(size: 16))
                            .foregroundColor(.white)
            ...
            // Continue — allows empty (user may have none)
            PeezyAssessmentButton(entries.isEmpty ? "None — Skip" : "Continue") {
                assessmentData.financialInstitutions = entries
        ...
        .onAppear {
            entries = assessmentData.financialInstitutions

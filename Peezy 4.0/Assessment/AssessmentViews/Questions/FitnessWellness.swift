...
struct FitnessWellness: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            GeometryReader { geo in
                VStack(spacing: 0) {
                    ...
                    HStack(spacing: 12) {
                        TextField("", text: $currentEntry, prompt: Text("e.g. Planet Fitness, Yoga studio").foregroundColor(.white.opacity(0.3)))
            ...
            PeezyAssessmentButton(entries.isEmpty ? "None — Skip" : "Continue") {
                assessmentData.fitnessWellness = entries
        ...
        .onAppear {
            entries = assessmentData.fitnessWellness

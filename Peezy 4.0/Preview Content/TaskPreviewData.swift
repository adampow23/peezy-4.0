...
enum TaskPreviewData {
    ...
    static let book_moversQualifying = WorkflowQualifying(
        workflowId: "book_movers",
        intro: WorkflowIntro(title: "Let's find you the right movers", subtitle: "A few quick questions so we can get you quotes from the top companies."),
        questions: [
            WorkflowQuestion(id: "heavy_items", question: "Any really heavy items?", subtitle: "These need special equipment.", options: [QuestionOption(id: "piano", label: "Piano / Organ", icon: "pianokeys"), QuestionOption(id: "safe", label: "Gun Safe / Safe", icon: "lock.shield"), QuestionOption(id: "hot_tub", label: "Hot Tub / Spa", icon: "drop.fill"), QuestionOption(id: "pool_table", label: "Pool Table", icon: "circle.grid.3x3")], type: .multi_select),
            WorkflowQuestion(id: "specialty_items", question: "Any delicate or high-value items?", subtitle: "These need extra care during transport.", options: [QuestionOption(id: "art", label: "Art / Antiques", icon: "photo.artframe"), QuestionOption(id: "glass", label: "Large Mirrors / Glass", icon: "rectangle"), QuestionOption(id: "wine", label: "Wine Collection", icon: "wineglass"), QuestionOption(id: "instruments", label: "Musical Instruments", icon: "guitars")], type: .multi_select),
            WorkflowQuestion(id: "packing_help", question: "Need help with packing?", subtitle: nil, options: [QuestionOption(id: "full", label: "Full service — pack everything", icon: "shippingbox.fill"), QuestionOption(id: "partial", label: "Just fragile / kitchen items", icon: "wineglass"), QuestionOption(id: "none", label: "No — I'll pack myself", icon: "hand.raised")], type: .single_select),
            WorkflowQuestion(id: "storage_needed", question: "Need storage?", subtitle: nil, options: [QuestionOption(id: "yes", label: "Yes", icon: "archivebox"), QuestionOption(id: "no", label: "No", icon: "xmark.circle")], type: .single_select),
            WorkflowQuestion(id: "storage_details", question: "Tell us about your storage needs", subtitle: nil, options: [QuestionOption(id: "5x5_partial", label: "Small (5×5) — partially full", icon: "square.grid.2x2"), QuestionOption(id: "5x5_full", label: "Small (5×5) — full", icon: "square.grid.2x2.fill"), QuestionOption(id: "10x10_partial", label: "Medium (10×10) — partially full", icon: "square.grid.3x3"), QuestionOption(id: "10x10_full", label: "Medium (10×10) — full", icon: "square.grid.3x3.fill"), QuestionOption(id: "10x20_partial", label: "Large (10×20) — partially full", icon: "rectangle.grid.2x2"), QuestionOption(id: "10x20_full", label: "Large (10×20) — full", icon: "rectangle.grid.2x2.fill")], type: .single_select),
            WorkflowQuestion(id: "insurance_context", question: "You're going to want to read this.", subtitle: "Industry standard moving coverage is only $0.60 per pound. That means a 50lb TV worth $800 would only be covered for $30. Full-value protection covers the actual replacement cost.", options: [], type: .single_select),
            WorkflowQuestion(id: "insurance_preference", question: "What level of coverage?", subtitle: nil, options: [QuestionOption(id: "full_value", label: "Full coverage — actual replacement value", icon: "shield.checkered"), QuestionOption(id: "basic", label: "Basic — standard $0.60/lb", icon: "shield"), QuestionOption(id: "none", label: "No insurance", icon: "xmark.shield")], type: .single_select)
        ],
        recap: WorkflowRecap(title: "Here's what we've got", closing: "Based on your answers, we'll find the top 3 companies and get you quotes from each. We'll reach out as soon as we have them.", button: "Request Quotes"),
        questionCount: 7
    )

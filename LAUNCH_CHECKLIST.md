...
| L15 | Day-one measurement layer. FirebaseCrashlytics and plain FirebaseAnalytics are linked; the three silent write failures report non-fatals; the 13-event funnel and `has_subscription` property are centralized with no PII parameters. Console evidence shows one received forced crash, the DEBUG journey dump contains every custom event exactly once, and the exact iPhone 17 Pro build passes. No backend or production deployment occurred. | machine | **VERIFIED** | Repeat local proof with `rg -n 'case .* = "' 'Peezy 4.0/MainInterface/Models/AnalyticsEvents.swift'`, inspect `Peezy 4.0/Documents/measurement-analytics-event-dump.txt` and `Peezy 4.0/Documents/measurement-crashlytics-receipt.png`, then run the exact build below. This closes measurement only: L11 remains OPEN and L12 BLOCKED until Adam approves the complete data-use matrix. FirebaseCrashlytics ships its own crash/diagnostic privacy declaration; the app manifest is intentionally not guessed or widened by this chip. |
...
| H08 | spawnTasks idempotency-token race (movers chain review, 2026-08-14). `executeSpawn` reads the token doc before the batch commits it, so two truly concurrent same-token calls could both create tasks. All current client flows serialize calls behind disabled buttons; risk is bounded. | machine | **DEFERRED** | Wrap the token claim + task creation in a Firestore transaction in `functions/spawnTasks.js`; add a concurrent same-token test to both spawnTasks suites proving exactly one task set is created; deploy the callable in a scoped release. Affects every spawn flow — do not fold into an unrelated change. |
| H09 | RENT_TRUCK dormant `onCompleteSpawns` metadata (movers chain review, 2026-08-14). The catalog field exists but no completion path executes it — the movers chain intentionally used direct SpawnService calls instead. | machine | **DEFERRED** | Either remove `onCompleteSpawns` from the RENT_TRUCK row in `functions/taskCatalogData.json` (plus the seeder/mapper decode when nothing else reads it) or wire RENT_TRUCK onto the movers-style flow-local spawn pattern. Reseed and run the exact iPhone 17 Pro build. |
| H10 | submitSupportMessage idempotency (movers chain review, 2026-08-14, user-deferred). The callable increments `unreadForAdmin` and fires a notification on every invocation, so the expert-review flow never retries it; a persisted-but-unconfirmed send shows honest "couldn't confirm delivery" copy instead. | machine | **DEFERRED** | Add a transactional `uid+messageId` claim in `functions/index.js` submitSupportMessage (increment/notify only on first claim); then the client may add a targeted same-message retry that reuses the messageId. Deploy in a scoped release with a replay test. |
| H11 | Unit-test host crashes at bootstrap (found 2026-08-14). Any `xcodebuild test` run of "Peezy 4.0Tests" dies before connecting: `PeezyV1App.init → SubscriptionManager.shared → Auth.auth()` assertion (Firebase not yet configured under the XCTest launch path). Pre-existing: an untouched suite (TaskGroupingTests) crashes identically and a matching `.ips` predates the movers-chain session. Four new Swift Testing suites (MoversChainCoordinatorTests, MoversMigrationTests, ExpertReviewTests, BookYourMoversTests) compile but cannot execute until this is fixed; pure suites in `Tests/` still run standalone via swiftc. | machine | **OPEN** | Guard the test-host launch: in `PeezyV1App.init`, configure Firebase before touching `SubscriptionManager.shared` when `NSClassFromString("XCTestCase") != nil` (or skip the eager StoreKit listener under test), without editing frozen `SubscriptionManager.swift`. Verify with `xcodebuild test -only-testing:"Peezy 4.0Tests/TaskGroupingTests"` then the four movers suites, all green. |

...
Generated from peezy-conventions-v2.md (audit @ f7e47ad + implementation through Spec 08 Phase D, 2026-07-27). Every fact below is code-verified unless explicitly labeled as remote evidence. Read peezy-conventions-v2.md and peezy-v1-architecture.md before spec work.
...
9. **The verify layer is implemented (Spec 08).** Post-move date gates surface factual check-in and box-return tasks. `submitCheckIn` durably writes reviews, produces deterministic flags/pending high strikes, and applies the confirmed-strike ladder; the box-return flow round-trips ordered/returned counts into `users/{uid}.kitCalibration`.
...
Single-decoder rule: both loaders call `PeezyCardFirestoreMapper.card()`. Never
add or re-inline a second Firestore→PeezyCard decoder.
...
| Vendor data | MainInterface/Models/Vendor.swift + functions/vendorsData.json | Backend-owned vendor/rate-card schema; strike arrays with legacy numeric decode; three placeholder movers; active filter gates downstream visibility |
| Vendor accountability | functions/accountabilityLadder.js + functions/submitCheckIn.js + docs/vendor-standards.md | Pure ladder, transactional review/strike writes, removal reconciliation, and vendor-facing standards |
...
| Box return | MainInterface/Models/BoxReturnService.swift + Tasks/Task Cards/BoxReturnView.swift | Durable ordered-kit count → `kitCalibration` read-back; optional existing-concierge pickup |
...
- Zero client-side webhook URLs. Mover/kit submissions and check-ins persist their durable Firestore records before direct best-effort Twilio notification. A missing value logs exact `SMS notify not configured` without rolling back the submission/review
...
- Catalog rows may carry `surfaceAfterDaysPastMove`. They cannot join the frozen dose without a move date or before moveDate + n. `MOVE_CHECKIN` uses +1; `BOX_RETURN` uses +7 and generation refresh additionally requires a durable `supplies_kit` response
- `submitCheckIn` writes `vendorReviews` as `{vendorId?, userId, answers, flags, submittedAt}`; general feedback uses `vendorId:null`. Exact flag SMS family: `PEEZY FLAG: {vendor} — {flag}.`
- Vendor strikes are arrays of `{date, source, severity, status, note}` with legacy numeric decode. Confirmed thresholds are 1 conversation, 2 warning, 3 removal; any confirmed `dayOfPriceChange` removes immediately. Console-only confirmation cannot run code, so Adam sets sibling `active=false` in the same vendor document when the confirmed state is removed
- `BOX_RETURN` reads delivered from the durable `supplies_kit` submitted JSON, writes and reads back `users/{uid}.kitCalibration {delivered, returned}`, and calls existing `requestConcierge` only when pickup is toggled with returned > 0
...
- Seed catalog: `cd functions && node seedTaskCatalog.js`. Deploy only targets a spec explicitly sanctions. **Remote evidence (Spec 08 acceptance):** mutations were limited to `functions:submitCheckIn` and the 47-row catalog reseed; Phase B redeployed only that callable, Phases C/D deployed nothing, and unchanged SMS families meant no `submitWorkflowAnswers` deploy
...
`LAUNCH_CHECKLIST.md` is the single source of truth for launch gates, owners,
evidence, exact close paths, deferred maintenance, and historical chips already
verified closed. Do not maintain a second checklist here.

...
START_PHASE=${1:-1}
DEFAULT_TURNS=40
EXPECTED_CATALOG_SHA256="6ebdc3378400d066447ce77c26e0227c0722b3650cda7015d1a09dab4081ef9d"
...
if (!expected.equals(actual)) {
  const withNewline = Buffer.concat([expected, Buffer.from('\n')]);
  if (!withNewline.equals(actual)) {
    throw new Error(
...
for i in $(seq "$START_PHASE" "$PHASES"); do
  ...
  PHASE_DEPLOY_POLICY="No deploy is authorized in this phase unless the phase text names a targeted deploy. Never run a broad Firebase Functions deploy. Never run node functions/seedTaskCatalog.js or any other live Firestore seed or write from inside a phase - validate seeding logic with a dry run or emulator only. Live seeding is a release-time step owned by the human."
  if [ "$i" -eq 2 ]; then

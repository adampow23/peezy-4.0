...
Source: taskCatalogData.json (47 live catalog rows in the locally validated and remotely reseeded source). This sheet preserves the original review verdicts, with later implementation state called out explicitly.
...
The original audit used ⚠ for then-broken condition keys. Those keys are now
collected; current implementation state is stated in each row below.
...
| BOOK_CLEANERS (55) | KEEP | SPINE | Full when vendors sign | Reweighted from 25 so booking surfaces with enough lead time |
| REMOVE_ITEMS (70) | KEEP | SPINE | Full when vendors sign | Reachable on `hasDeclutter=Yes` + `wantToSell=No`; both values are collected |
| SELL_ITEMS (75) | KEEP, low | LINK | Thin | Reachable on `hasDeclutter=Yes` + `wantToSell=Yes`; marketplace deep-links |
...
| UPDATE_AUTO_INSURANCE (82) | KEEP | RESOLVER | Full | `hasVehicles` is collected and normalized to the catalog's Yes/No contract |
| REGISTER_VEHICLE (82) | MERGE→DMV | see below | — | Original row retired into the two implemented DMV flows |
...
| TRANSFER_PHARMACY_RECORDS (75) | CORE | RESOLVER | Full | Reweighted from 55 because medication continuity needs earlier action |
...
| **ADD: MOVE_CHECKIN** | NEW | VERIFY | Full | Catalog row at moveDate+1; four factual questions → durable review, deterministic flags, strike ladder |
| **ADD: BOX_RETURN** | NEW | scheduled pickup | Thin | Catalog row at moveDate+7 for kit purchasers; ordered/returned calibration round-trip + optional concierge pickup |
...
## Urgency system — verified intent
The urgency scale doubles as reverse chronology (high = do early, low = do near move day) — RETURN_KEYS at 1 and DEFROST at 8 prove it is deliberate. The two reviewed reweights are complete: BOOK_CLEANERS 25→55 and TRANSFER_PHARMACY_RECORDS 55→75.

## Net result
Original Spec 02 projection: 56 tasks → **~40 catalog rows** after consolidation. Current implementation: **47 live catalog rows** after Spec 08 (45 pre-existing + `MOVE_CHECKIN` + `BOX_RETURN`); engine-generated packing work remains catalog-external. RESOLVER covers provider rows with a cited self-service path or concierge floor.

## Spec 02 consequences (locked once you sign off)
1. Persistence fixes restored hasVehicles + wantToSell/hasDeclutter + hasStorage; current conditions on those values are reachable
2. Merges implemented as catalog config (new taskIds, row-generation from multi-selects) — no App Store cycle
3. Two urgency reweights completed

...
- [ ] I3 accountDeletionFence.js: C3 global-scheduler cleanup step inside the application sweep (100/101 quarantine rows, cursor-map members, dueObservation, alert, ACCOUNT_DELETION_GLOBAL_PATH_MALFORMED); C6.1 registry static check over every existing file (taskDisposition.js/notificationIntents.js pending); phase2LegacyCreateBlocker handler + armed-only export + absence test (Decision 4).
